...
export default function CadmiumMetalLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return cdm_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#ca8a0422' }}>
          <Lock className="w-5 h-5" style={{ color: '#ca8a04' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Cadmium Metal Logistics</h2>
          <p className="text-sm text-gray-400">Cd NiCd battery &#8226; CdTe solar &#8226; QD display &#8226; Submarine NiCd propulsion supply chain</p>
        </div>
      </div>
