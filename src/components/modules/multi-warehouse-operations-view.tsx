...
export default function MultiWarehouseOperationsView() {
  ...
  const SortHeader = ({ field, children }: { field: string; children: React.ReactNode }) => (
    <Button variant="ghost" size="sm" className="press-scale mwo-sort-header h-8 px-2 text-[10px] font-semibold hover:bg-gray-100 dark:hover:bg-gray-800" onClick={() => handleSort(field)}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="mwo-tabs space-y-4">
        ...
        <TabsContent value="0" className="mwo-tab-content space-y-4">
          <div className="mwo-kpi-grid grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-4">{kpis.map((k, i) => (
            <Card key={i} className={`mwo-kpi-card group hover:shadow-md transition-all duration-300 ${k.bg}`}><CardContent className="inner-glow hover-lift-sm glass-subtle flex items-center gap-3 p-4"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-white shadow-sm ${k.color}`}><k.icon className="h-5 w-5" /></div><div className="min-w-0"><p className="text-[10px] font-medium text-gray-500 dark:text-gray-400 truncate">{k.label}</p><p className={`text-lg font-bold ${k.color}`}>{k.value}</p></div></CardContent></Card>
          ))}</div>
          <div className="mwo-chart-grid grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Card className="hover-lift-sm mwo-chart-card hover:shadow-lg transition-shadow duration-300"><CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Warehouse Capacity vs Utilization</CardTitle></CardHeader><CardContent><BarChart data={capBar}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} domain={[0, 100]} /><Tooltip /><Bar dataKey="Capacity" fill="#3b82f6" radius={[4, 4, 0, 0]} /><Bar dataKey="Utilization" fill="#059669" radius={[4, 4, 0, 0]} /></BarChart></CardContent></Card>
            ...
            <table className="mwo-wh-table w-full text-xs">
              ...
              <tbody>{filteredWH.map((wh) => (
                <tr key={wh.id} className="mwo-table-row border-b last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="press-scale p-2 text-center"><Button variant="ghost" size="sm" className="mwo-view-btn h-7 w-7 p-0 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/30" onClick={() => { setSelectedWH(wh); setSheetOpen(true); toast.success("Viewing Warehouse", `${wh.code} details`) }}><Eye className="h-3.5 w-3.5" /></Button></td>
                </tr>
              ))}</tbody>
            </table>
          </div>
