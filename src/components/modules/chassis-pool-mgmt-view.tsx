      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="0">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Card className="hover-lift-sm cpm-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Daily Utilization (30 days)</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={220}><AreaChart data={data.dailyUtil}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="day" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Legend wrapperStyle={{ fontSize: 11 }} /><Area type="monotone" dataKey="onRent" stackId="1" stroke="#0d9488" fill="#0d948840" /><Area type="monotone" dataKey="available" stackId="1" stroke="#059669" fill="#05966940" /><Area type="monotone" dataKey="maintenance" stackId="1" stroke="#d97706" fill="#d9770640" /></AreaChart></ResponsiveContainer></CardContent></Card>
            <Card className="hover-lift-sm cpm-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Port-wise Chassis</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={220}><BarChart data={data.portWise}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="port" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="count" fill="#334155" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></CardContent></Card>
            <Card className="hover-lift-sm cpm-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Chassis Type Distribution</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={220}><PieChart><Pie data={data.typeDist} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value" label={{ fontSize: 10 }}>{data.typeDist.map((_, i) => <Cell key={i} fill={data.PIE_COLORS[i % data.PIE_COLORS.length]} />)}</Pie><Tooltip /><Legend wrapperStyle={{ fontSize: 10 }} /></PieChart></ResponsiveContainer></CardContent></Card>
          </div>
        </TabsContent>
              ...
              return (<>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="press-scale cpm-action-btn flex-1 bg-slate-700 hover:bg-slate-800" onClick={() => { toast.success("Allocated", `${rec.chassisId} allocated`); setDrawerOpen(false) }}><Truck className="w-3.5 h-3.5 mr-1" />Allocate</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate cpm-action-btn flex-1" onClick={() => { toast.info("Maintenance", `${rec.chassisId} sent for maintenance`); setDrawerOpen(false) }}><Wrench className="w-3.5 h-3.5 mr-1" />Maintain</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate cpm-action-btn" onClick={() => { toast.info("Inspection", `${rec.chassisId} inspection scheduled`); setDrawerOpen(false) }}><Search className="w-3.5 h-3.5 mr-1" />Inspect</Button>
                  </div>
                </div>
              </>)
            })()}
            ...
            {drawerType === "alloc" && drawerRecord && (() => {
              ...
              return (<>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="press-scale cpm-action-btn flex-1 bg-teal-600 hover:bg-teal-700" onClick={() => { toast.success("Extended", `${rec.id} extended`); setDrawerOpen(false) }}><Clock className="w-3.5 h-3.5 mr-1" />Extend</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate cpm-action-btn flex-1" onClick={() => { toast.info("Returned", `${rec.id} returned`); setDrawerOpen(false) }}><RotateCcw className="w-3.5 h-3.5 mr-1" />Return</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate cpm-action-btn" onClick={() => { toast.warning("Shortfall", `${rec.id} shortfall reported`); setDrawerOpen(false) }}><AlertTriangle className="w-3.5 h-3.5 mr-1" />Shortfall</Button>
                  </div>
                </div>
              </>)
            })()}
            ...
            {drawerType === "maint" && drawerRecord && (() => {
              ...
              return (<>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="press-scale cpm-action-btn flex-1 bg-amber-600 hover:bg-amber-700" onClick={() => { toast.success("Approved", `${rec.id} approved`); setDrawerOpen(false) }}><CheckCircle2 className="w-3.5 h-3.5 mr-1" />Approve</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate cpm-action-btn flex-1" onClick={() => { toast.info("Rescheduled", `${rec.id} rescheduled`); setDrawerOpen(false) }}><CalendarDays className="w-3.5 h-3.5 mr-1" />Reschedule</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate cpm-action-btn" onClick={() => { toast.success("Completed", `${rec.id} completed`); setDrawerOpen(false) }}><CheckCircle2 className="w-3.5 h-3.5" /></Button>
                  </div>
                </div>
              </>)
            })()}
            ...
            {drawerType === "bill" && drawerRecord && (() => {
              ...
              return (<>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="press-scale cpm-action-btn flex-1 bg-indigo-600 hover:bg-indigo-700" onClick={() => { toast.success("Sent", `${rec.id} sent to customer`); setDrawerOpen(false) }}><Send className="w-3.5 h-3.5 mr-1" />Send</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate cpm-action-btn flex-1" onClick={() => { toast.success("Paid", `${rec.id} marked as paid`); setDrawerOpen(false) }}><CheckCircle2 className="w-3.5 h-3.5 mr-1" />Mark Paid</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate cpm-action-btn" onClick={() => { toast.warning("Disputed", `${rec.id} disputed`); setDrawerOpen(false) }}><AlertTriangle className="w-3.5 h-3.5 mr-1" />Dispute</Button>
                  </div>
                </div>
              </>)
            })()}
