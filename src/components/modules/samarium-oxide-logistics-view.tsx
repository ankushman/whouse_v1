...
export default function Samarium_OxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return samarium_oxide_RECORDS.filter(r => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#0f766e22' }}>
          <Sparkle className="w-5 h-5" style={{ color: '#0f766e' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Samarium Oxide Logistics</h2>
          <p className="text-sm text-gray-400">Sm2O3 SmCo magnet &#8226; Nuclear reactor &#8226; Catalyst &#8226; Optical glass supply chain</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Batches', value: kpis.total, color: '#0f766e' },
          { label: 'Investment (Cr)', value: kpis.invest, color: '#0f766e' },
          { label: 'Delayed', value: kpis.delayed, color: '#0f766e' },
          { label: 'Avg Purity', value: kpis.avgPurity + '%', color: '#0f766e' },
        ].map(k => (
            ...
            {zoneData.map(([zone, count]) => (
              <div key={zone} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#0f766e22', width: `${(count / kpis.total * 100).toFixed(0)}%` }} />
                </div>
          ...
          <div className="rounded-xl p-4 border border-gray-700/50 bg-gray-800/30">
            ...
            {filtered.slice(0, 8).map(r => (
              <div key={r[0]} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#0f766e22', width: `${Math.min(100, parseFloat(String(r[7]).replace(/[^0-9.]/g, '')) / 2)}%` }} />
                </div>
