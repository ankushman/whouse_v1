...
export default function AmmoniumVanadateLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return amv_RECORDS.filter(r => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold">Ammonium Vanadate Logistics</h2>
          <p className="text-sm text-gray-400">NH4VO3 catalyst &#8226; VRFB battery &#8226; Ferrovanadium &#8226; Submarine V-AIP fuel cell supply chain</p>
        </div>
      </div>
