...
export default function SiliconGermaniumLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return sge_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#4f46e522' }}>
          <Ruler className="w-5 h-5" style={{ color: '#4f46e5' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Silicon Germanium Logistics</h2>
          <p className="text-sm text-gray-400">SiGe RF BiCMOS &#8226; Space multi-junction PV &#8226; Thermal IR sensor &#8226; Strained-Si CMOS supply chain</p>
        </div>
      </div>
