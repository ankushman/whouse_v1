...
function KpiCard({ title, value, subtitle, icon: Icon, trend, color }: {
  ...
  return (
    <Card className="hover-lift-sm fmc-kpi-card">
            ...
            <>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" className="press-scale h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.info("Reassigning", `Reassigning ${(selectedItem as unknown as PickupOrder).pickupId}`); setDrawerOpen(false) }}><RefreshCw className="h-3 w-3" />Reassign</Button>
                  <Button size="sm" variant="outline" className="press-scale btn-outline-animate h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.success("Tracking", `Live tracking for ${(selectedItem as unknown as PickupOrder).pickupId}`) }}><Navigation className="h-3 w-3" />Track</Button>
                  <Button size="sm" variant="destructive" className="press-scale h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.error("Cancelled", `Pickup ${(selectedItem as unknown as PickupOrder).pickupId} cancelled`); setDrawerOpen(false) }}><XCircle className="h-3 w-3" />Cancel Pickup</Button>
                </div>
              </div>
            </>
          )}
          ...
          {drawerType === "route" && selectedItem && (
            <>
              ...
              <div className="p-6 space-y-4">
                ...
                <Separator />
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" className="press-scale h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.info("Optimizing", `Optimizing ${(selectedItem as unknown as RouteRecord).routeId}`); setDrawerOpen(false) }}><BarChart3 className="h-3 w-3" />Optimize</Button>
                  <Button size="sm" variant="outline" className="press-scale btn-outline-animate h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.success("Reassigned", `Route ${(selectedItem as unknown as RouteRecord).routeId} reassigned`) }}><RefreshCw className="h-3 w-3" />Reassign</Button>
                  <Button size="sm" variant="destructive" className="press-scale h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.warning("Closing", `Route ${(selectedItem as unknown as RouteRecord).routeId} closed`); setDrawerOpen(false) }}><XCircle className="h-3 w-3" />Close Route</Button>
                </div>
              </div>
            </>
          )}
          ...
          {drawerType === "driver" && selectedItem && (
            <>
              ...
              <div className="p-6 space-y-4">
                ...
                <Separator />
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" className="press-scale h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.success("Trip Assigned", `New trip for ${(selectedItem as unknown as DriverRecord).name}`) }}><Package className="h-3 w-3" />Assign Trip</Button>
                  <Button size="sm" variant="outline" className="press-scale btn-outline-animate h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.info("Calling", `Calling ${(selectedItem as unknown as DriverRecord).phone}`) }}><Phone className="h-3 w-3" />Contact</Button>
                  <Button size="sm" variant="destructive" className="press-scale h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.warning("Deactivated", `Driver ${(selectedItem as unknown as DriverRecord).name} deactivated`); setDrawerOpen(false) }}><XCircle className="h-3 w-3" />Deactivate</Button>
                </div>
              </div>
            </>
          )}
          ...
          {drawerType === "supplier" && selectedItem && (
            <>
              ...
              <div className="p-6 space-y-4">
                ...
                <Separator />
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" className="press-scale h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.success("Scheduled", `Pickup scheduled for ${(selectedItem as unknown as SupplierRecord).name}`) }}><CalendarDays className="h-3 w-3" />Schedule Pickup</Button>
                  <Button size="sm" variant="outline" className="press-scale btn-outline-animate h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.info("Audit", `Audit initiated for ${(selectedItem as unknown as SupplierRecord).name}`) }}><ShieldCheck className="h-3 w-3" />Audit</Button>
                  <Button size="sm" variant="destructive" className="press-scale h-8 text-xs gap-1 fmc-action-btn" onClick={() => { toast.warning("Deactivated", `Supplier ${(selectedItem as unknown as SupplierRecord).name} deactivated`); setDrawerOpen(false) }}><XCircle className="h-3 w-3" />Deactivate</Button>
                </div>
              </div>
            </>
          )}
