...
export default function Tungsten_OxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return tungsten_oxide_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#4338ca22' }}>
          <Cog className="w-5 h-5" style={{ color: '#4338ca' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Tungsten Oxide Logistics</h2>
          <p className="text-sm text-gray-400">WO3 smart window &#8226; Gas sensor &#8226; Fireproof &#8226; Tungsten carbide precursor supply chain</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Delayed', value: kpis.delayed, color: '#4338ca' },
