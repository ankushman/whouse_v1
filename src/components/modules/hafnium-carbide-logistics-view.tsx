...
export default function HafniumCarbideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return hfc_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#1e40af22' }}>
          <Bolt className="w-5 h-5" style={{ color: '#1e40af' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Hafnium Carbide Logistics</h2>
          <p className="text-sm text-gray-400">HfC UHTC TPS &#8226; Rocket nozzle &#8226; Fusion first-wall &#8226; Hypersonic glide vehicle supply chain</p>
        </div>
      </div>
            ...
            {zoneData.map(([zone, count]) => (
              <div key={zone} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#1e40af', width: `${(count / kpis.total * 100).toFixed(0)}%` }} />
                </div>
          ...
          <div className="rounded-xl p-4 border border-gray-700/50 bg-gray-800/30">
            ...
            {filtered.slice(0, 8).map(r => (
              <div key={r[0]} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#1e40af', width: `${Math.min(100, parseFloat(String(r[7]).replace(/[^0-9.]/g, '')) / 2)}%` }} />
                </div>
