...
export default function NickelCarbonateLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return nic_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#05966922' }}>
          <Cable className="w-5 h-5" style={{ color: '#059669' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Nickel Carbonate Logistics</h2>
          <p className="text-sm text-gray-400">NiCO3 stainless steel &#8226; Superalloy &#8226; NCA cathode &#8226; Submarine Monel propeller shaft supply chain</p>
        </div>
      </div>
