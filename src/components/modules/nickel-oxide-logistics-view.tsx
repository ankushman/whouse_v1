...
export default function Nickel_OxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return nickel_oxide_RECORDS.filter(r => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#7c3aed22' }}>
          <Magnet className="w-5 h-5" style={{ color: '#7c3aed' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Nickel Oxide Logistics</h2>
          <p className="text-sm text-gray-400">NiO thermistor &#8226; Ferromagnetic &#8226; Battery cathode &#8226; Catalyst supply chain</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Delayed', value: kpis.delayed, color: '#7c3aed' },
            ...
            {zoneData.map(([zone, count]) => (
              <div key={zone} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#7c3aed22', width: `${(count / kpis.total * 100).toFixed(0)}%` }} />
                </div>
          ...
          <div className="rounded-xl p-4 border border-gray-700/50 bg-gray-800/30">
            ...
            {filtered.slice(0, 8).map(r => (
              <div key={r[0]} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#7c3aed22', width: `${Math.min(100, parseFloat(String(r[7]).replace(/[^0-9.]/g, '')) / 2)}%` }} />
                </div>
