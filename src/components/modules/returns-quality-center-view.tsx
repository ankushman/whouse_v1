    ...
    <div className="space-y-4">
      ...
      {activeTab==="0" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="rqc-chart-card col-span-1"><CardHeader className="pb-1"><CardTitle className="text-xs">Inspection Trend</CardTitle></CardHeader><CardContent><LineChart data={data.inspTrend}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}}/><Tooltip/><Line type="monotone" dataKey="Passed" stroke={TH.emerald}/><Line type="monotone" dataKey="Failed" stroke={TH.rose}/><Line type="monotone" dataKey="Pending" stroke={TH.amber}/></LineChart></CardContent></Card>
            <Card className="rqc-chart-card col-span-1"><CardHeader className="pb-1"><CardTitle className="text-xs">Return Reasons</CardTitle></CardHeader><CardContent><PieChart><Pie data={data.reasonPie} cx="50%" cy="50%" innerRadius={40} outerRadius={70} dataKey="value" nameKey="name" label={false}>{data.reasonPie.map((_,i)=><Cell key={i} fill={PC[i%PC.length]}/>)}</Pie><Tooltip/></PieChart></CardContent></Card>
            <Card className="rqc-chart-card col-span-1"><CardHeader className="pb-1"><CardTitle className="text-xs">Category Quality</CardTitle></CardHeader><CardContent><BarChart data={data.catScore}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name" tick={{fontSize:9}} angle={-45}/><YAxis tick={{fontSize:10}}/><Tooltip/><Bar dataKey="score" fill={TH.rose} radius={[4,4,0,0]}/></BarChart></CardContent></Card>
          </div>
        </div>
      )}
      ...
      {activeTab==="5" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="rqc-chart-card"><CardHeader className="pb-1"><CardTitle className="text-xs">Pass Rate Trend</CardTitle></CardHeader><CardContent><AreaChart data={data.passTrend}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} domain={[80,100]}/><Tooltip/><Area type="monotone" dataKey="rate" stroke={TH.emerald} fill="rgba(5,150,105,0.15)"/><Line type="monotone" dataKey="target" stroke={TH.amber} strokeDasharray="5 5"/></AreaChart></CardContent></Card>
          <Card className="rqc-chart-card"><CardHeader className="pb-1"><CardTitle className="text-xs">Defect Distribution</CardTitle></CardHeader><CardContent><PieChart><Pie data={data.defectPie} cx="50%" cy="50%" innerRadius={40} outerRadius={70} dataKey="value" nameKey="name" label={false}>{data.defectPie.map((_,i)=><Cell key={i} fill={PC[i%PC.length]}/>)}</Pie><Tooltip/></PieChart></CardContent></Card>
          <Card className="rqc-chart-card"><CardHeader className="pb-1"><CardTitle className="text-xs">Category Quality</CardTitle></CardHeader><CardContent><BarChart data={data.catQuality}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name" tick={{fontSize:9}} angle={-45}/><YAxis tick={{fontSize:10}} domain={[60,100]}/><Tooltip/><Bar dataKey="score" fill={TH.blue} radius={[4,4,0,0]}/></BarChart></CardContent></Card>
          <Card className="rqc-chart-card"><CardHeader className="pb-1"><CardTitle className="text-xs">Chargeback Recovery</CardTitle></CardHeader><CardContent><LineChart data={data.recoveryTrend}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} domain={[30,100]}/><Tooltip/><Line type="monotone" dataKey="recovery" stroke={TH.indigo} strokeWidth={2} dot={{r:3}}/></LineChart></CardContent></Card>
        </div>
      )}
