...
export default function GermaniumDioxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return ged_RECORDS.filter(r => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#ca8a0422' }}>
          <Cylinder className="w-5 h-5" style={{ color: '#ca8a04' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Germanium Dioxide Logistics</h2>
          <p className="text-sm text-gray-400">GeO2 IR optical lens &#8226; Fiber optic preform &#8226; Submarine periscope &#8226; 5G photonics supply chain</p>
        </div>
      </div>
