...
const molybdenum_oxide_RECORDS = [
...
export default function Molybdenum_OxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return molybdenum_oxide_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#05966922' }}>
          <Factory className="w-5 h-5" style={{ color: '#059669' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Molybdenum Oxide Logistics</h2>
          <p className="text-sm text-gray-400">MoO3 steel alloy &#8226; Catalyst &#8226; Pigment &#8226; Lubricant supply chain</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Delayed', value: kpis.delayed, color: '#059669' },
