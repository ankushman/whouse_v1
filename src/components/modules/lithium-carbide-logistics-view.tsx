...
export default function Lithium_CarbideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return lithium_carbide_RECORDS.filter(r => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#f59e0b22' }}>
          <Flame className="w-5 h-5" style={{ color: '#f59e0b' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Lithium Carbide Logistics</h2>
          <p className="text-sm text-gray-400">Li2C2 tritium breeder &#8226; Nuclear fusion &#8226; Battery chemistry &#8226; High-energy supply chain</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Batches', value: kpis.total, color: '#f59e0b' },
          { label: 'Investment (Cr)', value: kpis.invest, color: '#f59e0b' },
          { label: 'Delayed', value: kpis.delayed, color: '#ef4444' },
          { label: 'Avg Purity', value: kpis.avgPurity + '%', color: '#f59e0b' },
        ].map(k => (
            ...
            {zoneData.map(([zone, count]) => (
              <div key={zone} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#f59e0b', width: `${(count / kpis.total * 100).toFixed(0)}%` }} />
                </div>
