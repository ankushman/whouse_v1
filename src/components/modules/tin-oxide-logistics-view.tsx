...
export default function Tin_OxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return tin_oxide_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#6366f122' }}>
          <Lightbulb className="w-5 h-5" style={{ color: '854d0e' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Tin Oxide Logistics</h2>
          <p className="text-sm text-gray-400">SnO2 TCO electrode &#8226; Gas sensor &#8226; Display &#8226; Smart glass supply chain</p>
        </div>
      </div>
