...
export default function BariumFluorideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return baf_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#6d28d922' }}>
          <Space className="w-5 h-5" style={{ color: '#6d28d9' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Barium Fluoride Logistics</h2>
          <p className="text-sm text-gray-400">BaF2 fast scintillator &#8226; EUV lithography optic &#8226; IR transparent window &#8226; Neutron detection supply chain</p>
        </div>
      </div>
