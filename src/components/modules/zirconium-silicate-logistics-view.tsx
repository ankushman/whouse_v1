...
export default function ZirconiumSilicateLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return zrs_RECORDS.filter(r => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#4d7c0f22' }}>
          <Martini className="w-5 h-5" style={{ color: '#4d7c0f' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Zirconium Silicate Logistics</h2>
          <p className="text-sm text-gray-400">ZrSiO4 foundry casting &#8226; Ceramic opacifier &#8226; Thermal barrier &#8226; Dental ceramic supply chain</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Batches', value: kpis.total, color: '#4d7c0f' },
          { label: 'Investment (Cr)', value: kpis.invest, color: '#4d7c0f' },
          { label: 'Delayed', value: kpis.delayed, color: '#ef4444' },
          { label: 'Avg Purity', value: kpis.avgPurity + '%', color: '#4d7c0f' },
        ].map(k => (
            ...
            {zoneData.map(([zone, count]) => (
              <div key={zone} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#4d7c0f', width: `${(count / kpis.total * 100).toFixed(0)}%` }} />
                </div>
          ...
          <div className="rounded-xl p-4 border border-gray-700/50 bg-gray-800/30">
            ...
            {filtered.slice(0, 8).map(r => (
              <div key={r[0]} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#4d7c0f', width: `${Math.min(100, parseFloat(String(r[7]).replace(/[^0-9.]/g, '')) / 2)}%` }} />
                </div>
