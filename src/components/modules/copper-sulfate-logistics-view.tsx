...
export default function CopperSulfateLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return cus_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#b4530922' }}>
          <BadgeCheck className="w-5 h-5" style={{ color: '#b45309' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Copper Sulfate Logistics</h2>
          <p className="text-sm text-gray-400">CuSO4 fungicide &#8226; Electroplating &#8226; Mining flotation &#8226; Submarine anti-fouling paint supply chain</p>
        </div>
      </div>
