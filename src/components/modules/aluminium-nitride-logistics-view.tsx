...
const aluminium_nitride_RECORDS = [
...
export default function Aluminium_NitrideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return aluminium_nitride_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#6366f122' }}>
          <Microchip className="w-5 h-5" style={{ color: '#6366f1' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Aluminium Nitride Logistics</h2>
          <p className="text-sm text-gray-400">AlN ceramic substrate &#8226; Thermal management &#8226; High-frequency RF &#8226; Defense electronics supply chain</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Batches', value: kpis.total, color: '#6366f1' },
          { label: 'Investment (Cr)', value: kpis.invest, color: '#6366f1' },
          { label: 'Delayed', value: kpis.delayed, color: '#ef4444' },
          { label: 'Avg Purity', value: kpis.avgPurity + '%', color: '#6366f1' },
        ].map(k => (
            ...
            {zoneData.map(([zone, count]) => (
              <div key={zone} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#6366f1', width: `${(count / kpis.total * 100).toFixed(0)}%` }} />
                </div>
