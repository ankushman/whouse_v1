...
export default function LeadZirconateLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return pbz_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#4338ca22' }}>
          <BrickWall className="w-5 h-5" style={{ color: '#4338ca' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Lead Zirconate Logistics</h2>
          <p className="text-sm text-gray-400">PbZrO3 FeRAM ferroelectric &#8226; Submarine sonar piezo &#8226; PZT actuator &#8226; Towed array hydrophone supply chain</p>
        </div>
      </div>
