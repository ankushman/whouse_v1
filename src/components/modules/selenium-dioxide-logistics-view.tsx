...
export default function SeleniumDioxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return sed_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#a1620722' }}>
          <Citrus className="w-5 h-5" style={{ color: '#a16207' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Selenium Dioxide Logistics</h2>
          <p className="text-sm text-gray-400">SeO2 photoreceptor &#8226; Glass decolorizer &#8226; CZTSSe solar &#8226; Submarine optronic mast supply chain</p>
        </div>
      </div>
