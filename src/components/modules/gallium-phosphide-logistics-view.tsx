...
export default function GalliumPhosphideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return gap_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#05966922' }}>
          <Coffee className="w-5 h-5" style={{ color: '#059669' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Gallium Phosphide Logistics</h2>
          <p className="text-sm text-gray-400">GaP green LED wafer &#8226; UV photodetector &#8226; Space solar cell &#8226; Radiation-hard IC supply chain</p>
        </div>
      </div>
