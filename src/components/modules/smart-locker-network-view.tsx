    ...
    <div className="space-y-4">
      ...
      {activeTab==="5" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="sln-chart-card"><CardHeader className="pb-1"><CardTitle className="text-xs">Revenue Trend</CardTitle></CardHeader><CardContent><LineChart data={data.revenueTrend}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}}/><Tooltip/><Line type="monotone" dataKey="revenue" stroke={TH.indigo} strokeWidth={2} dot={{r:3}}/></LineChart></CardContent></Card>
          <Card className="sln-chart-card"><CardHeader className="pb-1"><CardTitle className="text-xs">City Utilization</CardTitle></CardHeader><CardContent><BarChart data={data.cityUtil} layout="vertical"><CartesianGrid strokeDasharray="3 3"/><XAxis type="number" tick={{fontSize:10}}/><YAxis dataKey="city" type="category" tick={{fontSize:9}} width={80}/><Tooltip/><Bar dataKey="utilization" fill={TH.blue} radius={[0,4,4,0]}/></BarChart></CardContent></Card>
          <Card className="sln-chart-card"><CardHeader className="pb-1"><CardTitle className="text-xs">Carrier Distribution</CardTitle></CardHeader><CardContent><PieChart><Pie data={data.carrierPie} cx="50%" cy="50%" innerRadius={40} outerRadius={70} dataKey="value" nameKey="name" label={false}>{data.carrierPie.map((_,i)=><Cell key={i} fill={PC[i%PC.length]}/>)}</Pie><Tooltip/></PieChart></CardContent></Card>
          <Card className="sln-chart-card"><CardHeader className="pb-1"><CardTitle className="text-xs">Dwell Time Pattern</CardTitle></CardHeader><CardContent><AreaChart data={data.dwellDist}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="hour" tick={{fontSize:9}}/><YAxis tick={{fontSize:10}}/><Tooltip/><Area type="monotone" dataKey="deposits" stroke={TH.emerald} fill="rgba(5,150,105,0.2)"/><Area type="monotone" dataKey="pickups" stroke={TH.blue} fill="rgba(59,130,246,0.2)"/></AreaChart></CardContent></Card>
        </div>
      )}
