...
export default function TinMetalLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return snm_RECORDS.filter(r => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold">Tin Metal Logistics</h2>
          <p className="text-sm text-gray-400">Sn SMT solder &#8226; Float glass &#8226; Bronze bearing &#8226; Submarine sonar dome supply chain</p>
        </div>
      </div>
