...
export default function ChromiumSulfateLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return crs_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#7c3aed22' }}>
          <Bug className="w-5 h-5" style={{ color: '#7c3aed' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Chromium Sulfate Logistics</h2>
          <p className="text-sm text-gray-400">Cr2(SO4)3 leather tanning &#8226; Chromic anodize &#8226; Refractory &#8226; Submarine non-magnetic hull supply chain</p>
        </div>
      </div>
