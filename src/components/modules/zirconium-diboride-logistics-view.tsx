...
export default function ZirconiumDiborideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return zrb_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#be123c22' }}>
          <PlugZap className="w-5 h-5" style={{ color: '#be123c' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Zirconium Diboride Logistics</h2>
          <p className="text-sm text-gray-400">ZrB2 hypersonic TPS &#8226; Nuclear fuel cladding &#8226; Plasma electrode &#8226; UHT ceramic supply chain</p>
        </div>
      </div>
