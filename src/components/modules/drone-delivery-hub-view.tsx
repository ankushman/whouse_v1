        ...
        <TabsContent value="0" className="space-y-4 mt-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {kpis.map((k, i) => { const Icon = k.icon; return (
              <Card key={i} className="ddh-kpi-card border-l-4 border-l-sky-500 hover:shadow-lg transition-shadow">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <Card className="ddh-chart-card col-span-2"><CardHeader className="pb-2"><CardTitle className="text-sm">Hourly Deliveries</CardTitle></CardHeader><CardContent><AreaChart data={data.hourlyDeliveries} height={240}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="hour" tick={{ fontSize: 8 }} interval={2} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Area type="monotone" dataKey="Completed" stackId="a" fill="#059669" /><Area type="monotone" dataKey="In-Flight" stackId="a" fill="#0284c7" /><Area type="monotone" dataKey="Charging" stackId="a" fill="#d97706" /></AreaChart></CardContent></Card>
            <Card className="ddh-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Drone Types</CardTitle></CardHeader><CardContent><PieChart width={240} height={240}><Pie data={data.typeDist} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, percent }) => `${name.split("-")[0]} ${(percent * 100).toFixed(0)}%`} labelLine={false} fontSize={9}>{data.typeDist.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}</Pie><Tooltip /></PieChart></CardContent></Card>
          </div>
          <Card className="ddh-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Zone Coverage</CardTitle></CardHeader><CardContent><BarChart data={data.zoneCoverage} height={200}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="value" fill="#0284c7" radius={[4, 4, 0, 0]} /></BarChart></CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="5" className="space-y-4 mt-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <Card key={i} className="ddh-kpi-card border-l-4 border-l-emerald-500"><CardContent className="p-3"><div className="flex items-center justify-between"><div><p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-wider">{k.label}</p><p className="text-base font-bold mt-0.5">{k.value}</p></div><Icon className={cn("w-5 h-5 opacity-50", k.color)} /></div></CardContent></Card>
            )})}
          </div>
          <div className="grid md:grid-cols-2 gap-4">
            <Card className="ddh-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Daily Delivery Trend</CardTitle></CardHeader><CardContent><LineChart data={data.dailyTrend} height={240}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="day" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Line type="monotone" dataKey="deliveries" stroke="#0284c7" strokeWidth={2} /></LineChart></CardContent></Card>
            <Card className="ddh-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Zone Throughput</CardTitle></CardHeader><CardContent><BarChart data={data.zoneThroughput} height={240}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="count" fill="#059669" radius={[4, 4, 0, 0]} /></BarChart></CardContent></Card>
          </div>
          <div className="grid md:grid-cols-2 gap-4">
            <Card className="ddh-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Failure Reasons</CardTitle></CardHeader><CardContent><BarChart data={data.failureReasons} layout="vertical" height={260}><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" tick={{ fontSize: 10 }} /><YAxis type="category" dataKey="name" tick={{ fontSize: 9 }} width={110} /><Tooltip /><Bar dataKey="count" fill="#e11d48" radius={[0, 4, 4, 0]} /></BarChart></CardContent></Card>
            <Card className="ddh-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Cost vs Revenue (6-Month)</CardTitle></CardHeader><CardContent><AreaChart data={data.costTrend} height={260}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} tickFormatter={(v: number) => `${(v / 100000).toFixed(0)}L`} /><Tooltip formatter={(v: number) => formatINR(v)} /><Area type="monotone" dataKey="Fuel Savings" stackId="a" fill="#059669" /><Area type="monotone" dataKey="Labor Savings" stackId="a" fill="#0284c7" /><Area type="monotone" dataKey="Maintenance" stackId="a" fill="#ea580c" /><Area type="monotone" dataKey="Revenue" stackId="a" fill="#7c3aed" /></AreaChart></CardContent></Card>
          </div>
        </TabsContent>
      </Tabs>
          ...
          {selectedDelivery && (<>
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="flex gap-2">
                <Button size="sm" className="text-xs flex-1 bg-sky-600 hover:bg-sky-700" onClick={() => toast.success("Dispatched", `Delivery ${selectedDelivery.id} dispatched`)}>Dispatch</Button>
                <Button size="sm" className="text-xs flex-1 bg-emerald-600 hover:bg-emerald-700" onClick={() => toast.info("Rerouted", `Delivery ${selectedDelivery.id} rerouted`)}>Reroute</Button>
              </div>
            </div>
          </>)}
        </SheetContent>
