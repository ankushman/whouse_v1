...
export default function Yttrium_OxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return yttrium_oxide_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#6366f122' }}>
          <Star className="w-5 h-5" style={{ color: 'dc2626' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Yttrium Oxide Logistics</h2>
          <p className="text-sm text-gray-400">Y2O3 YAG laser host &#8226; Phosphor &#8226; Thermal barrier &#8226; Superconductor supply chain</p>
        </div>
      </div>
