...
export default function BismuthMetalLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return bsm_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#be185d22' }}>
          <Pill className="w-5 h-5" style={{ color: '#be185d' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Bismuth Metal Logistics</h2>
          <p className="text-sm text-gray-400">Bi pharma compound &#8226; Fusible alloy &#8226; Lead-free solder &#8226; Submarine EMI shield supply chain</p>
        </div>
      </div>
