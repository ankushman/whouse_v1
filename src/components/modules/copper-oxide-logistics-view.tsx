...
export default function Copper_OxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return copper_oxide_RECORDS.filter(r => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#ea580c22' }}>
          <CircuitBoard className="w-5 h-5" style={{ color: '#ea580c' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Copper Oxide Logistics</h2>
          <p className="text-sm text-gray-400">CuO semiconductor &#8226; Antifouling paint &#8226; Battery cathode &#8226; PCB etching supply chain</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Batches', value: kpis.total, color: '#ea580c' },
          { label: 'Investment (Cr)', value: kpis.invest, color: '#ea580c' },
          { label: 'Delayed', value: kpis.delayed, color: '#ea580c' },
          { label: 'Avg Purity', value: kpis.avgPurity + '%', color: '#ea580c' },
        ].map(k => (
            ...
            {zoneData.map(([zone, count]) => (
              <div key={zone} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#ea580c22', width: `${(count / kpis.total * 100).toFixed(0)}%` }} />
                </div>
          ...
          <div className="rounded-xl p-4 border border-gray-700/50 bg-gray-800/30">
            ...
            {filtered.slice(0, 8).map(r => (
              <div key={r[0]} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-gray-700/50">
                  <div className="h-2 rounded-full" style={{ backgroundColor: '#ea580c22', width: `${Math.min(100, parseFloat(String(r[7]).replace(/[^0-9.]/g, '')) / 2)}%` }} />
                </div>
