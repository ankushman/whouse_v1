...
export default function ManganeseSulfateLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return mns_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#15803d22' }}>
          <Cherry className="w-5 h-5" style={{ color: '#15803d' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Manganese Sulfate Logistics</h2>
          <p className="text-sm text-gray-400">MnSO4 fertilizer &#8226; NMC cathode &#8226; Animal feed &#8226; Submarine Pb-MnO2 battery supply chain</p>
        </div>
      </div>
