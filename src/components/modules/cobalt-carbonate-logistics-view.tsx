...
export default function CobaltCarbonateLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return cbc_RECORDS.filter(r => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold">Cobalt Carbonate Logistics</h2>
          <p className="text-sm text-gray-400">CoCO3 NMC cathode &#8226; Superalloy binder &#8226; Ceramic pigment &#8226; Submarine sonar magnet supply chain</p>
        </div>
      </div>
