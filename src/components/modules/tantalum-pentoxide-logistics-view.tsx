...
export default function TantalumPentoxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return tpt_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#b91c1c22' }}>
          <HandMetal className="w-5 h-5" style={{ color: '#b91c1c' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Tantalum Pentoxide Logistics</h2>
          <p className="text-sm text-gray-400">Ta2O5 MLCC dielectric &#8226; DRAM capacitor &#8226; Submarine sonar &#8226; Quantum qubit supply chain</p>
        </div>
      </div>
