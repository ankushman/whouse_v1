...
export default function CadmiumSulfideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return cds_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#7c3aed22' }}>
          <Binoculars className="w-5 h-5" style={{ color: '#7c3aed' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Cadmium Sulfide Logistics</h2>
          <p className="text-sm text-gray-400">CdS thin-film PV &#8226; Photoresistor sensor &#8226; Cadmium yellow pigment &#8226; Quantum dot solar supply chain</p>
        </div>
      </div>
