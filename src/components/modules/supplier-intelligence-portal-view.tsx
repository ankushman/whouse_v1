    ...
    <div className="sip-root flex flex-col gap-4 p-4 md:p-6">
      <PageHeader title="Supplier Intelligence Portal" description="AI-powered supplier analytics, risk intelligence & contract management for Indian logistics" actions={
        <div className="flex gap-2">
          ...
          <Button size="sm" variant="outline" className="btn-outline-animate gap-1 text-xs sip-btn" onClick={() => exportToCSV(filtered, `supplier-intelligence-tab${activeTab}`)}><FileDown className="h-3 w-3" />Export</Button>
        </div>
      } />
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="0" className="sip-tab-dashboard flex flex-col gap-4 mt-4">
          <div className="glass-subtle grid grid-cols-2 md:grid-cols-4 gap-3">{kpiDefs.map((k, i) => <Card key={i} className="sip-kpi"><CardContent className="p-3"><div className="flex items-center justify-between"><span className="text-[11px] text-muted-foreground">{k.label}</span><span className={cn("sip-kpi-icon", k.color)}>{k.icon}</span></div><p className="text-lg font-bold mt-1">{k.value}</p></CardContent></Card>)}</div>
        ...
        <TabsContent value="3" className="sip-tab-risks flex flex-col gap-4 mt-4">
          ...
          <Card className="numeric-cell glass-subtle sip-table-card overflow-hidden"><CardContent className="p-0 overflow-x-auto"><Table><TableHeader><TableRow><SortHeader col="id" label="ID" /><SortHeader col="supplier" label="Supplier" /><SortHeader col="type" label="Type" /><SortHeader col="severity" label="Severity" /><SortHeader col="score" label="Score" /><SortHeader col="mitigation" label="Mitigation" /><SortHeader col="impact" label="Impact" /><SortHeader col="probability" label="Prob" /><SortHeader col="state" label="State" /><TableHead className="text-xs w-10" /></TableRow></TableHeader><TableBody>{filtered.slice(0, 25).map((r, i) => <TableRow key={i} className="sip-table-row hover:bg-accent/40 cursor-pointer" onClick={() => { setSelectedRow(r); setSheetOpen(true) }}><TableCell className="text-xs font-mono">{String(r.id)}</TableCell><TableCell className="text-xs font-medium max-w-[120px] truncate">{String(r.supplier)}</TableCell><TableCell><RiskTypeBadge t={String(r.type)} /></TableCell><TableCell><RiskSeverityBadge s={String(r.severity)} /></TableCell><TableCell><RiskScoreBar v={Number(r.score)} /></TableCell><TableCell className="text-xs">{String(r.mitigation)}</TableCell><TableCell><ImpactTile v={Number(r.impact)} /></TableCell><TableCell className="text-xs">{String(r.probability)}%</TableCell><TableCell className="text-xs text-muted-foreground">{String(r.state)}</TableCell><TableCell><Button size="icon" variant="ghost" className="h-7 w-7" onClick={e => { e.stopPropagation(); setSelectedRow(r); setSheetOpen(true) }}><Eye className="h-3.5 w-3.5" /></Button></TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
        </TabsContent>
        ...
        <SheetContent side="right" className="sip-sheet w-[440px] overflow-y-auto p-0">
          ...
          <div className="p-5 space-y-1">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button size="sm" variant="outline" className="btn-outline-animate sip-action h-8 text-xs gap-1" onClick={() => toast.info("Exported", "Record exported to CSV")}><FileDown className="h-3 w-3" />Export</Button>
