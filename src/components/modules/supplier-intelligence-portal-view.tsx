...
// 1
const SupplierCategoryBadge = ({ cat }: { cat: string }) => <Badge className="badge-interactive sip-cat-badge text-[11px] px-2 py-0 bg-violet-100 text-violet-800 dark:bg-violet-900/40 dark:text-violet-300">{CAT_EMOJI[cat] ?? ""} {cat}</Badge>
// 2
...
// 4
const CityBadge = ({ city }: { city: string }) => <Badge variant="outline" className="badge-interactive sip-city-badge text-[11px] px-2 py-0"><Globe className="h-3 w-3 mr-1" />{city}</Badge>
// 5
...
const MetricTypeBadge = ({ m }: { m: string }) => <Badge className="badge-interactive sip-metric-badge text-[11px] px-2 py-0 bg-indigo-100 text-indigo-800 dark:bg-indigo-900/40 dark:text-indigo-300">{MI[m]} {m}</Badge>
// 6
...
const RiskTypeBadge = ({ t }: { t: string }) => <Badge className="badge-interactive sip-risk-type-badge text-[11px] px-2 py-0 bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-300">{RI[t]} {t}</Badge>
// 9
...
// 11
const CTTypeBadge = ({ t }: { t: string }) => <Badge className="badge-interactive sip-ct-type-badge text-[11px] px-2 py-0 bg-violet-100 text-violet-800 dark:bg-violet-900/40 dark:text-violet-300">{t}</Badge>
// 12
    ...
    <div className="sip-root flex flex-col gap-4 p-4 md:p-6">
      <PageHeader title="Supplier Intelligence Portal" description="AI-powered supplier analytics, risk intelligence & contract management for Indian logistics" actions={
        <div className="flex gap-2">
          <Button size="sm" className="press-scale gap-1 text-xs sip-btn" onClick={() => toast.success("Refreshed", "Supplier data refreshed successfully")}><RefreshCw className="h-3 w-3" />Refresh</Button>
          <Button size="sm" variant="outline" className="press-scale btn-outline-animate gap-1 text-xs sip-btn" onClick={() => exportToCSV(filtered, `supplier-intelligence-tab${activeTab}`)}><FileDown className="h-3 w-3" />Export</Button>
        </div>
      } />
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="0" className="sip-tab-dashboard flex flex-col gap-4 mt-4">
          <div className="inner-glow hover-lift-sm glass-subtle grid grid-cols-2 md:grid-cols-4 gap-3">{kpiDefs.map((k, i) => <Card key={i} className="sip-kpi"><CardContent className="p-3"><div className="flex items-center justify-between"><span className="text-[11px] text-muted-foreground">{k.label}</span><span className={cn("sip-kpi-icon", k.color)}>{k.icon}</span></div><p className="text-lg font-bold mt-1">{k.value}</p></CardContent></Card>)}</div>
          <div className="grid md:grid-cols-2 gap-4">
            <Card className="hover-lift-sm sip-chart"><CardHeader className="pb-1"><CardTitle className="text-sm">Monthly Spend Trend</CardTitle><CardDescription className="text-[11px]">Direct, Indirect & Spot (\u20b9 L)</CardDescription></CardHeader><CardContent><LineChart data={data.monthlySpend} height={230}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" tick={{ fontSize: 11 }} /><YAxis tick={{ fontSize: 11 }} /><Tooltip /><Line type="monotone" dataKey="direct" stroke={C.blue} strokeWidth={2} /><Line type="monotone" dataKey="indirect" stroke={C.emerald} strokeWidth={2} /><Line type="monotone" dataKey="spot" stroke={C.amber} strokeWidth={2} /></LineChart></CardContent></Card>
            <Card className="hover-lift-sm sip-chart"><CardHeader className="pb-1"><CardTitle className="text-sm">Supplier Categories</CardTitle><CardDescription className="text-[11px]">Distribution across 8 categories</CardDescription></CardHeader><CardContent><PieChart height={230}><Pie data={data.catPie} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value" label={({ name }: { name: string }) => <span className="text-[10px]">{name}</span>} labelLine={false}>{data.catPie.map((_, i) => <Cell key={i} fill={CC[i % CC.length]} />)}</Pie><Tooltip /></PieChart></CardContent></Card>
          </div>
          <Card className="hover-lift-sm sip-chart"><CardHeader className="pb-1"><CardTitle className="text-sm">Top 10 Supplier Performance</CardTitle><CardDescription className="text-[11px]">Quality vs Delivery scores</CardDescription></CardHeader><CardContent><BarChart data={data.perfBar} height={250}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 10 }} angle={-25} textAnchor="end" height={55} /><YAxis tick={{ fontSize: 11 }} /><Tooltip /><Bar dataKey="quality" fill={C.blue} /><Bar dataKey="delivery" fill={C.emerald} /></BarChart></CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="3" className="sip-tab-risks flex flex-col gap-4 mt-4">
          ...
          <Card className="inner-glow press-scale hover-lift-sm numeric-cell glass-subtle sip-table-card overflow-hidden"><CardContent className="p-0 overflow-x-auto"><Table><TableHeader><TableRow><SortHeader col="id" label="ID" /><SortHeader col="supplier" label="Supplier" /><SortHeader col="type" label="Type" /><SortHeader col="severity" label="Severity" /><SortHeader col="score" label="Score" /><SortHeader col="mitigation" label="Mitigation" /><SortHeader col="impact" label="Impact" /><SortHeader col="probability" label="Prob" /><SortHeader col="state" label="State" /><TableHead className="text-xs w-10" /></TableRow></TableHeader><TableBody>{filtered.slice(0, 25).map((r, i) => <TableRow key={i} className="sip-table-row hover:bg-accent/40 cursor-pointer" onClick={() => { setSelectedRow(r); setSheetOpen(true) }}><TableCell className="text-xs font-mono">{String(r.id)}</TableCell><TableCell className="text-xs font-medium max-w-[120px] truncate">{String(r.supplier)}</TableCell><TableCell><RiskTypeBadge t={String(r.type)} /></TableCell><TableCell><RiskSeverityBadge s={String(r.severity)} /></TableCell><TableCell><RiskScoreBar v={Number(r.score)} /></TableCell><TableCell className="text-xs">{String(r.mitigation)}</TableCell><TableCell><ImpactTile v={Number(r.impact)} /></TableCell><TableCell className="text-xs">{String(r.probability)}%</TableCell><TableCell className="text-xs text-muted-foreground">{String(r.state)}</TableCell><TableCell><Button size="icon" variant="ghost" className="h-7 w-7" onClick={e => { e.stopPropagation(); setSelectedRow(r); setSheetOpen(true) }}><Eye className="h-3.5 w-3.5" /></Button></TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
        </TabsContent>
        ...
        <SheetContent side="right" className="sip-sheet w-[440px] overflow-y-auto p-0">
          ...
          <div className="p-5 space-y-1">
            ...
            <div className="flex flex-wrap gap-2">
              <Button size="sm" className="press-scale sip-action h-8 text-xs gap-1" onClick={() => toast.success("Updated", `Record ${String(selectedRow?.id)} updated`)}><Zap className="h-3 w-3" />Update</Button>
              <Button size="sm" variant="outline" className="press-scale btn-outline-animate sip-action h-8 text-xs gap-1" onClick={() => toast.info("Exported", "Record exported to CSV")}><FileDown className="h-3 w-3" />Export</Button>
              <Button size="sm" variant="destructive" className="press-scale sip-action h-8 text-xs gap-1" onClick={() => { toast.error("Archived", `Record ${String(selectedRow?.id)} archived`); setSheetOpen(false) }}><AlertTriangle className="h-3 w-3" />Archive</Button>
            </div>
          </div>
        </SheetContent>
      </Sheet>
