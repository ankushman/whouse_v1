...
export default function NiobiumPentoxideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return nbp_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#7e22ce22' }}>
          <Pentagon className="w-5 h-5" style={{ color: '#7e22ce' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Niobium Pentoxide Logistics</h2>
          <p className="text-sm text-gray-400">Nb2O5 MLCC dielectric &#8226; LiNbO3 substrate &#8226; Submarine sonar &#8226; Hypersonic TPS supply chain</p>
        </div>
      </div>
