...
export default function ZincSulfateLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return zns_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#0e749022' }}>
          <Map className="w-5 h-5" style={{ color: '#0e7490' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Zinc Sulfate Logistics</h2>
          <p className="text-sm text-gray-400">ZnSO4 fertilizer &#8226; Zn-ion battery &#8226; Galvanizing bath &#8226; Submarine hull CP supply chain</p>
        </div>
      </div>
