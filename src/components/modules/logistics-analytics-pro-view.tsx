...
export default function LogisticsAnalyticsProView() {
  ...
  const SortHeader = ({ field, children }: { field: string; children: React.ReactNode }) => (
    <Button variant="ghost" size="sm" className="press-scale lap-sort-header h-8 px-2 text-[10px] font-semibold hover:bg-gray-100 dark:hover:bg-gray-800" onClick={() => handleSort(field)}>
            ...
            <table className="lap-metric-table w-full text-xs">
              ...
              <tbody>{filteredMetrics.map((m) => (
                <tr key={m.id} className="lap-table-row border-b last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="press-scale p-2 text-center"><Button variant="ghost" size="sm" className="lap-view-btn h-7 w-7 p-0 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/30" onClick={() => { setSelectedMetric(m); setSheetOpen(true); toast.success("Viewing Metric", `${m.id} details`) }}><Eye className="h-3.5 w-3.5" /></Button></td>
                </tr>
              ))}</tbody>
            </table>
          </div>
        ...
        <TabsContent value="2" className="lap-tab-content space-y-4">
          <div className="lap-forecast-grid grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
            {data.forecasts.map((fc) => (
              <Card key={fc.id} className="hover-lift-sm lap-forecast-card group hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 overflow-hidden border-l-4 border-l-violet-500">
