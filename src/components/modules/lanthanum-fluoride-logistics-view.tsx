...
export default function LanthanumFluorideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return ltf_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#0f766e22' }}>
          <Shell className="w-5 h-5" style={{ color: '#0f766e' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Lanthanum Fluoride Logistics</h2>
          <p className="text-sm text-gray-400">LaF3 DUV optical coating &#8226; Fluoride ion battery &#8226; Fiber laser host &#8226; Scintillator supply chain</p>
        </div>
      </div>
