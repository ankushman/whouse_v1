...
function DoorTypeBadge({ type }: { type: string }) {
  ...
  return <Badge className="ddo-type-badge text-xs px-2 py-0.5" style={{ background: c + "18", color: c, border: `1px solid ${c}30` }}>{type}</Badge>
}
function ZoneBadge({ zone }: { zone: string }) {
  const short = zone.split(" - ")[0]
  const c = ZONE_COLORS[short] || "#475569"
  return <Badge variant="outline" className="ddo-zone-badge text-xs px-2 py-0.5" style={{ borderColor: c + "60", color: c }}>{zone}</Badge>
}
...
function PriorityBadge({ priority }: { priority: string }) {
  ...
  return <Badge className="ddo-priority-badge text-xs px-2 py-0.5" style={{ background: c + "18", color: c, border: `1px solid ${c}30` }}>{priority}</Badge>
}
function CarrierBadge({ carrier }: { carrier: string }) {
  return <Badge className="ddo-carrier-badge text-xs px-2 py-0.5 bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300 border border-blue-200 dark:border-blue-800">{carrier}</Badge>
}
function ApptTypeBadge({ type }: { type: string }) {
  return <Badge variant="outline" className="ddo-appt-type text-xs px-2 py-0.5 border-indigo-300 dark:border-indigo-700 text-indigo-600 dark:text-indigo-400">{type}</Badge>
}
function TruckBadge({ truck }: { truck: string }) {
  return <Badge variant="outline" className="ddo-truck-badge text-xs px-2 py-0.5 border-slate-300 dark:border-slate-600 font-mono">{truck}</Badge>
}
function AlertSeverityBadge({ severity }: { severity: string }) {
  ...
  return <Badge className="ddo-severity-badge text-xs px-2 py-0.5" style={{ background: c + "18", color: c, border: `1px solid ${c}30` }}>{severity}</Badge>
}
function EquipmentBadge({ equip }: { equip: string }) {
  return <Badge variant="outline" className="ddo-equip-badge text-xs px-2 py-0.5 border-slate-300 dark:border-slate-600">{equip}</Badge>
}
...
function WeatherBadge({ weather }: { weather: string }) {
  ...
  return <Badge variant="outline" className="ddo-weather text-xs px-2 py-0.5 border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-400">{icons[weather] || "🌤️"} {weather}</Badge>
}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            {data.rules.map((r, i) => (
              <Card key={i} className={cn("ddo-rule-card", r.status === "Active" && "border-l-2 border-emerald-500")}>
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><Badge className="text-xs font-mono bg-slate-100 dark:bg-slate-800 px-2 py-0.5">{r.id}</Badge><StatusBadge status={r.status} color={data.STATUS_COLORS[r.status] || "#475569"} /></div>
                    <PriorityBadge priority={r.priority} />
                  </div>
                  <p className="text-sm font-semibold">{r.ruleName}</p>
                  <div className="flex items-center gap-2 text-xs"><span className="text-muted-foreground">Type:</span><Badge variant="outline" className="px-2 py-0.5 border-slate-300 dark:border-slate-600">{r.ruleType}</Badge></div>
                  ...
                  <div className="flex gap-2">
                    {r.status === "Active" ? (
                      <Button size="sm" variant="outline" className="ddo-action-btn" onClick={() => toast.info("Paused", `Rule ${r.id} paused`)}><Pause className="w-3.5 h-3.5 mr-1" />Pause</Button>
                    ) : r.status === "Paused" ? (
                    ...
                    {r.status !== "Archived" && <Button size="sm" variant="outline" className="ddo-action-btn" onClick={() => toast.info("Editing", `Rule ${r.id} edit mode`)}><Settings className="w-3.5 h-3.5" /></Button>}
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <TabsContent value="5">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Card className="ddo-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Equipment Status</CardTitle></CardHeader><CardContent className="space-y-2">{data.equipment.map((e, i) => (<div key={i} className="flex items-center gap-3 p-2 rounded-lg bg-slate-50/50 dark:bg-slate-800/50"><EquipmentBadge equip={e.equipment} /><div className="flex-1"><div className="flex items-center justify-between"><span className="text-xs font-medium">{e.equipment}</span><span className={cn("text-xs tabular-nums", e.status === "Online" && "text-emerald-600", e.status === "Offline" && "text-red-600")}>{e.status}</span></div><Progress value={e.utilization} className="h-1.5 mt-1" /><span className="text-xs text-muted-foreground">{e.utilization}%</span></div></div>))}</CardContent></Card>
          </div>
        </TabsContent>
      </Tabs>
          ...
          <>
              ...
              return (<>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="outline" className="ddo-action-btn flex-1" onClick={() => { toast.info("Maintenance", `Door ${rec.doorId} maintenance`); setDrawerOpen(false) }}><Wrench className="w-3.5 h-3.5 mr-1" />Maintenance</Button>
                    <Button size="sm" variant="outline" className="ddo-action-btn" onClick={() => { toast.success("Released", `Door ${rec.doorId} released`); setDrawerOpen(false) }}><CheckCircle2 className="w-3.5 h-3.5" />Release</Button>
                  </div>
                </div>
              </>)
            })()}
            ...
            {drawerType === "alert" && drawerRecord && (() => {
              ...
              return (<>
                ...
                <div className="p-4 space-y-4">
                  <div className="flex items-center gap-2 flex-wrap"><AlertSeverityBadge severity={rec.severity} /><Badge variant="outline" className={cn("text-xs", rec.resolved === "Resolved" && "border-emerald-300 text-emerald-700")}>{rec.resolved}</Badge></div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="outline" className="ddo-action-btn flex-1" onClick={() => { toast.warning("Escalated", `Alert ${rec.id} escalated`); setDrawerOpen(false) }}><ArrowUpRight className="w-3.5 h-3.5 mr-1" />Escalate</Button>
                    <Button size="sm" variant="outline" className="ddo-action-btn" onClick={() => { toast.info("Ignored", `Alert ${rec.id} ignored`); setDrawerOpen(false) }}><Ban className="w-3.5 h-3.5" />Ignore</Button>
                  </div>
                </div>
              </>)
            })()}
