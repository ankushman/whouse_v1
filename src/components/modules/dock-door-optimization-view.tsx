      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="0">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Card className="hover-lift-sm ddo-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Hourly Throughput (16h)</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={220}><AreaChart data={data.hourlyThroughput}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="hour" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Legend wrapperStyle={{ fontSize: 11 }} /><Area type="monotone" dataKey="inbound" stackId="1" stroke="#3b82f6" fill="#3b82f640" /><Area type="monotone" dataKey="outbound" stackId="1" stroke="#059669" fill="#05966940" /><Area type="monotone" dataKey="crossDock" stackId="1" stroke="#d97706" fill="#d9770640" /></AreaChart></ResponsiveContainer></CardContent></Card>
            <Card className="hover-lift-sm ddo-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Zone Utilization</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={220}><BarChart data={data.zoneUtil}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="zone" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="utilization" fill="#3b82f6" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></CardContent></Card>
            <Card className="hover-lift-sm ddo-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Carrier On-Time Performance</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={220}><BarChart data={data.carrierPerf} layout="vertical"><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" tick={{ fontSize: 10 }} /><YAxis dataKey="carrier" type="category" tick={{ fontSize: 10 }} width={85} /><Tooltip /><Bar dataKey="onTime" fill="#059669" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer></CardContent></Card>
          </div>
        </TabsContent>
              ...
              <Card key={i} className={cn("ddo-rule-card", r.status === "Active" && "border-l-2 border-emerald-500")}>
                <CardContent className="inner-glow glass-subtle p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    {r.status === "Active" ? (
                      <Button size="sm" variant="outline" className="press-scale btn-outline-animate ddo-action-btn" onClick={() => toast.info("Paused", `Rule ${r.id} paused`)}><Pause className="w-3.5 h-3.5 mr-1" />Pause</Button>
                    ) : r.status === "Paused" ? (
                      <Button size="sm" className="press-scale ddo-action-btn flex-1 bg-emerald-600 hover:bg-emerald-700" onClick={() => toast.success("Activated", `Rule ${r.id} activated`)}><Play className="w-3.5 h-3.5 mr-1" />Activate</Button>
                    ) : null}
                    {r.status !== "Archived" && <Button size="sm" variant="outline" className="press-scale btn-outline-animate ddo-action-btn" onClick={() => toast.info("Editing", `Rule ${r.id} edit mode`)}><Settings className="w-3.5 h-3.5" /></Button>}
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <TabsContent value="5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 ddo-shimmer">
            {data.metrics.map((m, i) => (
              <Card key={i} className="hover-lift-sm ddo-metric-card" style={{ borderLeftColor: PIE_COLORS[i % PIE_COLORS.length] }}>
                <CardContent className="inner-glow glass-subtle p-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Card className="hover-lift-sm ddo-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Simulation Scenarios</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={220}><BarChart data={data.simScenarios}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="scenario" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="throughput" fill="#3b82f6" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></CardContent></Card>
            <Card className="hover-lift-sm ddo-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Weather Impact</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={220}><BarChart data={data.weatherImpact} layout="vertical"><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" tick={{ fontSize: 10 }} domain={[0, 100]} /><YAxis dataKey="condition" type="category" tick={{ fontSize: 10 }} width={90} /><Tooltip /><Bar dataKey="impactPct" fill="#d97706" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer></CardContent></Card>
            <Card className="inner-glow hover-lift-sm glass-subtle ddo-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Equipment Status</CardTitle></CardHeader><CardContent className="space-y-2">{data.equipment.map((e, i) => (<div key={i} className="flex items-center gap-3 p-2 rounded-lg bg-slate-50/50 dark:bg-slate-800/50"><EquipmentBadge equip={e.equipment} /><div className="flex-1"><div className="flex items-center justify-between"><span className="text-xs font-medium">{e.equipment}</span><span className={cn("text-xs tabular-nums", e.status === "Online" && "text-emerald-600", e.status === "Offline" && "text-red-600")}>{e.status}</span></div><Progress value={e.utilization} className="h-1.5 mt-1" /><span className="text-xs text-muted-foreground">{e.utilization}%</span></div></div>))}</CardContent></Card>
          </div>
        </TabsContent>
      </Tabs>
            ...
            {drawerType === "door" && drawerRecord && (() => {
              ...
              return (<>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="press-scale ddo-action-btn flex-1 bg-blue-600 hover:bg-blue-700" onClick={() => { toast.success("Assigned", `Door ${rec.doorId} assigned`); setDrawerOpen(false) }}><Truck className="w-3.5 h-3.5 mr-1" />Assign</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate ddo-action-btn flex-1" onClick={() => { toast.info("Maintenance", `Door ${rec.doorId} maintenance`); setDrawerOpen(false) }}><Wrench className="w-3.5 h-3.5 mr-1" />Maintenance</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate ddo-action-btn" onClick={() => { toast.success("Released", `Door ${rec.doorId} released`); setDrawerOpen(false) }}><CheckCircle2 className="w-3.5 h-3.5" />Release</Button>
                  </div>
                </div>
              </>)
            })()}
            ...
            {drawerType === "appt" && drawerRecord && (() => {
              ...
              return (<>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="press-scale ddo-action-btn flex-1 bg-teal-600 hover:bg-teal-700" onClick={() => { toast.success("Started", `Appt ${rec.id} started`); setDrawerOpen(false) }}><Play className="w-3.5 h-3.5 mr-1" />Start</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate ddo-action-btn flex-1" onClick={() => { toast.info("Redirected", `Appt ${rec.id} redirected`); setDrawerOpen(false) }}><Navigation className="w-3.5 h-3.5 mr-1" />Redirect</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate ddo-action-btn" onClick={() => { toast.warning("Cancelled", `Appt ${rec.id} cancelled`); setDrawerOpen(false) }}><XCircle className="w-3.5 h-3.5" />Cancel</Button>
                  </div>
                </div>
              </>)
            })()}
            ...
            {drawerType === "alert" && drawerRecord && (() => {
              ...
              return (<>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className={cn("ddo-action-btn flex-1", rec.resolved !== "Resolved" && "bg-emerald-600 hover:bg-emerald-700")} onClick={() => { toast.success("Resolved", `Alert ${rec.id} resolved`); setDrawerOpen(false) }}><CheckCircle2 className="press-scale w-3.5 h-3.5 mr-1" />Resolve</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate ddo-action-btn flex-1" onClick={() => { toast.warning("Escalated", `Alert ${rec.id} escalated`); setDrawerOpen(false) }}><ArrowUpRight className="w-3.5 h-3.5 mr-1" />Escalate</Button>
                    <Button size="sm" variant="outline" className="press-scale btn-outline-animate ddo-action-btn" onClick={() => { toast.info("Ignored", `Alert ${rec.id} ignored`); setDrawerOpen(false) }}><Ban className="w-3.5 h-3.5" />Ignore</Button>
                  </div>
                </div>
              </>)
            })()}
