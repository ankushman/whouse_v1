...
export default function AntimonyMetalLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return sbm_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#dc262622' }}>
          <Compass className="w-5 h-5" style={{ color: '#dc2626' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Antimony Metal Logistics</h2>
          <p className="text-sm text-gray-400">Sb battery grid &#8226; PET catalyst &#8226; Flame retardant &#8226; Submarine battery supply chain</p>
        </div>
      </div>
