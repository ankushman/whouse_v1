...
export default function CeriumFluorideLogisticsView() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [activeTab, setActiveTab] = useState('dashboard');

  const filtered = useMemo(() => {
    return cef_RECORDS.filter(r => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg" style={{ backgroundColor: '#b4530922' }}>
          <Clover className="w-5 h-5" style={{ color: '#b45309' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold">Cerium Fluoride Logistics</h2>
          <p className="text-sm text-gray-400">CeF3 gamma scintillator &#8226; UV transparent optic &#8226; Glass polish &#8226; Catalytic washcoat supply chain</p>
        </div>
      </div>
