...
export function KPICard({ title, value, change = 0, trend = "neutral", icon: Icon, subtitle, index = 0, colorClass = "bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400", breakdown, sparklineData }: KPICardProps) {
  ...
  const cardContent = (
    <Card className={cn(
      "group kpi-shimmer card-hover-glow stat-card-highlight card-depth card-morph shadow-card relative overflow-hidden rounded-xl border border-border/60 bg-card transition-smooth hover:shadow-card-hover hover:border-border cursor-pointer card-lift hover-scale-sm"
    )}>
