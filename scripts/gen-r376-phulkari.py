#!/usr/bin/env python3
"""Generate R376 Phulkari Embroidery Punjab module (253 lines)"""
import os

BASE = "/home/z/my-project/src/components/modules"
FILE = os.path.join(BASE, "phulkari-embroidery-punjab-logistics-view.tsx")
...
function KpiTile({ label, value, unit }: { label: string; value: number; unit: string }) {
  return <Card><CardContent className="phk-kpi"><div style={{ fontSize: 13, color: "#6b7280" }}>{label}</div><div style={{ fontSize: 26, fontWeight: 800, color: "#b45309" }}>{value.toLocaleString()}<span style={{ fontSize: 13, fontWeight: 400 }}>{unit}</span></div></div></Card>;
}

function ValueTile({ label, value }: { label: string; value: string }) {
  return <Card><CardContent className="phk-value"><div style={{ fontSize: 13, color: "#6b7280" }}>{label}</div><div style={{ fontSize: 18, fontWeight: 700, color: "#1f2937" }}>{value}</div></div></Card>;
}
