...
const header = L([
  ...
  `import { AreaChart, Area, BarChart, Bar, PieChart, Pie, Cell, LineChart, Line, XAxis, YAxis, CartesianGrid, Legend, Tooltip, ResponsiveContainer } from "recharts"`,
  `import {`,
  ...
  `  Building2, Navigation, Fuel, Layers, CircleDot, Settings, Tool, Zap, Ban,`,
  `} from "lucide-react"`,
]);
...
// ============================================================================`,
  ...
  `const LOCATIONS = ["JNPT Mumbai", "Mundra Gujarat", "Chennai Tamil Nadu", "Hazira Gujarat", "ICD Tughlakabad Delhi", "ICD Patparganj Delhi", "ICD Nagpur", "ICD Bengaluru"] as const`,
...
content += L([
  ...
  `  cost: "Cost", startDate: "Start", endDate: "End", maintType: "Type",`,
...
// ============================================================================`,
  ...
  `      amount: amt,`,
  `      tax: Math.round(amt * (pick(["0.18", "0.12", "0.05", "0.28"], seed + 5) as unknown as number)),`,
...
// ============================================================================`,
  ...
  `  return <Badge variant="outline" className={cn("cpm-badge text-xs px-2 py-0.5 border", isPulse && "cpm-pulse-error")} style={{ borderColor: color, color }}><span className="inline-block w-1.5 h-1.5 rounded-full mr-1" style={{ backgroundColor: color }} />{status}</Badge>`,
  `}`,
  ...
  `  return <div className="cpm-tire-bar flex items-center gap-2"><div className="h-2 rounded-full bg-gray-200 dark:bg-gray-700 w-full overflow-hidden"><div className="h-full rounded-full transition-all" style={{ width: \`\${pct}%\`, background: \`linear-gradient(90deg, \${c}, \${c}cc)\` }} /></div><span className="text-xs font-mono tabular-nums" style={{ color: c }}>{pct}%</span></div>`,
  `}`,
  ...
  `  return <div className="cpm-pickup-drop flex items-center gap-1.5 text-xs"><span className="truncate max-w-[100px]">{from}</span><ChevronRight className="w-3 h-3 text-slate-400 shrink-0" /><span className="truncate max-w-[100px]">{to}</span></div>`,
  `}`,
  ...
  `  return <div className="cpm-tax-tile flex gap-2 text-xs"><span className="bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 px-1.5 py-0.5 rounded">CGST {formatINR(cgst)}</span><span className="bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300 px-1.5 py-0.5 rounded">SGST {formatINR(sgst)}</span></div>`,
  `}`,
...
content += L([
  ...
  `            {kpis.map((kpi, i) => (`,
  `              <Card key={i} className={cn("cpm-kpi cpm-kpi-anim", i === 0 && "cpm-kpi-slate", i === 1 && "cpm-kpi-emerald", i === 2 && "cpm-kpi-teal", i === 3 && "cpm-kpi-amber", i === 4 && "cpm-kpi-indigo", i === 5 && "cpm-kpi-cyan", i === 6 && "cpm-kpi-rose", i === 7 && "cpm-kpi-orange")}>`,
  `                <CardContent className="p-4"><div className="flex items-center justify-between"><div><p className="text-xs text-muted-foreground">{kpi.label}</p><p className="text-xl font-bold tabular-nums mt-1">{kpi.value}</p></div><kpi.icon className="w-8 h-8 opacity-20" style={{ color: kpi.color }} /></div></CardContent>`,
  `              </Card>`,
  `            ))}`,
  `          </div>`,
...
content += L([
  ...
  `        <TabsContent value="5">`,
  `          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 cpm-shimmer">`,
  `            {[{ label: "Total Revenue", value: formatINR(data.analytics.totalRevenue), color: "#334155" },{ label: "Avg Daily Rate", value: formatINR(data.analytics.avgDailyRate), color: "#0d9488" },{ label: "Utilization Rate", value: \`\${data.analytics.utilizationRate}%\`, color: "#4f46e5" },{ label: "Maint. Cost", value: formatINR(data.analytics.maintenanceCost), color: "#e11d48" },{ label: "Availability Index", value: \`\${data.analytics.availabilityIndex}%\`, color: "#059669" },{ label: "Turnaround", value: \`\${data.analytics.turnaroundTime}d\`, color: "#d97706" },{ label: "Fleet Age", value: \`\${data.analytics.fleetAge}yr\`, color: "#ea580c" },{ label: "Satisfaction", value: \`\${data.analytics.customerSatisfaction}%\`, color: "#0891b2" }].map((kpi, i) => (`,
  `              <Card key={i} className={cn("cpm-analytics-card", i === 0 && "cpm-kpi-slate", i === 1 && "cpm-kpi-teal", i === 2 && "cpm-kpi-indigo", i === 3 && "cpm-kpi-rose", i === 4 && "cpm-kpi-emerald", i === 5 && "cpm-kpi-amber", i === 6 && "cpm-kpi-orange", i === 7 && "cpm-kpi-cyan")} style={{ borderLeftColor: kpi.color }}><CardContent className="p-4"><p className="text-xs text-muted-foreground">{kpi.label}</p><p className="text-lg font-bold tabular-nums mt-1">{kpi.value}</p></CardContent></Card>`,
  `            ))}`,
  `          </div>`,
  `          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">`,
  ...
  `            <Card className="cpm-chart-card"><CardHeader className="pb-2"><CardTitle className="text-sm">Utilization by Location</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={220}><BarChart data={data.utilByLoc} layout="vertical"><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" tick={{ fontSize: 10 }} /><YAxis dataKey="location" type="category" tick={{ fontSize: 10 }} width={70} /><Tooltip /><Bar dataKey="utilization" fill="#0d9488" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer></CardContent></Card>`,
...
content += L([
  ...
  `                  <div className="grid grid-cols-2 gap-3 text-xs"><div><span className="text-muted-foreground">Serial No</span><div className="font-mono font-semibold">{rec.serial}</div></div><div><span className="text-muted-foreground">Year</span><div className="font-medium">{rec.year}</div></div><div><span className="text-muted-foreground">Owner</span><div className="font-medium">{rec.owner}</div></div><div><span className="text-muted-foreground">KM Traveled</span><div className="font-medium tabular-nums">{rec.kmTraveled.toLocaleString("en-IN")}</div></div><div className="col-span-2"><span className="text-muted-foreground">Tire Condition</span><div className="mt-1"><TireBar pct={rec.tirePct} /></div></div><div><span className="text-muted-foreground">Last Inspection</span><div className="font-medium">{rec.lastInspection}</div></div><div><span className="text-muted-foreground">ID</span><div className="font-mono font-semibold">{rec.id}</div></div></div>`,
  `                  <div className="flex gap-2"><Button size="sm" className="cpm-action-btn flex-1 bg-slate-700 hover:bg-slate-800" onClick={() => { toast.success("Allocated", \`${rec.chassisId} allocated\`); setDrawerOpen(false) }}><Truck className="w-3.5 h-3.5 mr-1" />Allocate</Button><Button size="sm" variant="outline" className="cpm-action-btn flex-1" onClick={() => { toast.info("Maintenance", \`${rec.chassisId} sent for maintenance\`); setDrawerOpen(false) }}><Wrench className="w-3.5 h-3.5 mr-1" />Maintain</Button><Button size="sm" variant="outline" className="cpm-action-btn" onClick={() => { toast.info("Inspection", \`${rec.chassisId} inspection scheduled\`); setDrawerOpen(false) }}><Search className="w-3.5 h-3.5 mr-1" />Inspect</Button></div>`,
  `                </div>`,
  `              </>)`,
  `            })()}`,
  ``,
  ...
  `                  <div className="grid grid-cols-2 gap-3 text-xs"><div><span className="text-muted-foreground">Allocation ID</span><div className="font-mono font-semibold">{rec.id}</div></div><div><span className="text-muted-foreground">Type</span><div className="font-medium">{rec.allocationType}</div></div><div><span className="text-muted-foreground">Cost</span><div className="font-bold tabular-nums">{formatINR(rec.cost)}</div></div><div><span className="text-muted-foreground">Chassis</span><div className="font-mono">{rec.chassisId}</div></div><div><span className="text-muted-foreground">Start</span><div className="font-medium">{rec.startDate}</div></div><div><span className="text-muted-foreground">End</span><div className="font-medium">{rec.endDate}</div></div></div>`,
  `                  <div className="flex gap-2"><Button size="sm" className="cpm-action-btn flex-1 bg-teal-600 hover:bg-teal-700" onClick={() => { toast.success("Extended", \`${rec.id} extended\`); setDrawerOpen(false) }}><Clock className="w-3.5 h-3.5 mr-1" />Extend</Button><Button size="sm" variant="outline" className="cpm-action-btn flex-1" onClick={() => { toast.info("Returned", \`${rec.id} returned\`); setDrawerOpen(false) }}><RotateCcw className="w-3.5 h-3.5 mr-1" />Return</Button><Button size="sm" variant="outline" className="cpm-action-btn" onClick={() => { toast.warning("Shortfall", \`${rec.id} shortfall reported\`); setDrawerOpen(false) }}><AlertTriangle className="w-3.5 h-3.5 mr-1" />Shortfall</Button></div>`,
  `                </div>`,
  `              </>)`,
  `            })()}`,
  ``,
  ...
  `                  <div className="grid grid-cols-2 gap-3 text-xs"><div><span className="text-muted-foreground">Record ID</span><div className="font-mono font-semibold">{rec.id}</div></div><div><span className="text-muted-foreground">Type</span><div className="font-medium">{rec.maintType}</div></div><div><span className="text-muted-foreground">Parts Cost</span><div className="font-bold tabular-nums">{formatINR(rec.partsCost)}</div></div><div><span className="text-muted-foreground">Labor Hours</span><div className="font-medium tabular-nums">{rec.laborHrs}h</div></div><div><span className="text-muted-foreground">Scheduled</span><div className="font-medium">{rec.scheduledDate}</div></div><div><span className="text-muted-foreground">Next Due</span><div className="font-medium">{rec.nextDue}</div></div><div className="col-span-2"><span className="text-muted-foreground">Chassis</span><div className="font-mono">{rec.chassisId}</div></div></div>`,
  `                  <div className="flex gap-2"><Button size="sm" className="cpm-action-btn flex-1 bg-amber-600 hover:bg-amber-700" onClick={() => { toast.success("Approved", \`${rec.id} approved\`); setDrawerOpen(false) }}><CheckCircle2 className="w-3.5 h-3.5 mr-1" />Approve</Button><Button size="sm" variant="outline" className="cpm-action-btn flex-1" onClick={() => { toast.info("Rescheduled", \`${rec.id} rescheduled\`); setDrawerOpen(false) }}><CalendarDays className="w-3.5 h-3.5 mr-1" />Reschedule</Button><Button size="sm" variant="outline" className="cpm-action-btn" onClick={() => { toast.success("Completed", \`${rec.id} completed\`); setDrawerOpen(false) }}><CheckCircle2 className="w-3.5 h-3.5" /></Button></div>`,
  `                </div>`,
  `              </>)`,
  `            })()}`,
  ``,
  ...
  `                  <div className="grid grid-cols-2 gap-3 text-xs"><div><span className="text-muted-foreground">Bill ID</span><div className="font-mono font-semibold">{rec.id}</div></div><div><span className="text-muted-foreground">Charge Type</span><div className="font-medium">{rec.chargeType}</div></div><div><span className="text-muted-foreground">Amount</span><div className="font-bold tabular-nums text-lg">{formatINR(rec.amount)}</div></div><div><span className="text-muted-foreground">Tax</span><div><TaxTile tax={rec.tax} /></div></div><div><span className="text-muted-foreground">Due Date</span><div className="font-medium"><DueDateIndicator date={rec.dueDate} /></div></div><div><span className="text-muted-foreground">Paid Date</span><div className="font-medium">{rec.paidDate}</div></div><div className="col-span-2"><span className="text-muted-foreground">Chassis</span><div className="font-mono">{rec.chassisId}</div></div></div>`,
  `                  <div className="flex gap-2"><Button size="sm" className="cpm-action-btn flex-1 bg-indigo-600 hover:bg-indigo-700" onClick={() => { toast.success("Sent", \`${rec.id} sent to customer\`); setDrawerOpen(false) }}><Send className="w-3.5 h-3.5 mr-1" />Send</Button><Button size="sm" variant="outline" className="cpm-action-btn flex-1" onClick={() => { toast.success("Paid", \`${rec.id} marked as paid\`); setDrawerOpen(false) }}><CheckCircle2 className="w-3.5 h-3.5 mr-1" />Mark Paid</Button><Button size="sm" variant="outline" className="cpm-action-btn" onClick={() => { toast.warning("Disputed", \`${rec.id} disputed\`); setDrawerOpen(false) }}><AlertTriangle className="w-3.5 h-3.5 mr-1" />Dispute</Button></div>`,
  `                </div>`,
  `              </>)`,
  `            })()}`,
  `          </>`,
...
// Fix: ArrowUpDown not imported in lucide — use existing imports
// Actually let me add it to the import
const fixedHeader = header.replace(
  `import {`,
  `import { ArrowUpDown,`
);

const finalContent = content.replace(header + '\n', fixedHeader + '\n');

fs.writeFileSync('/home/z/my-project/src/components/modules/chassis-pool-mgmt-view.tsx', finalContent);
console.log(`Written ${finalContent.split('\\n').length} lines`);
