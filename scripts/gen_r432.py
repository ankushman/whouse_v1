#!/usr/bin/env python3
"""R432 Generator: Hafnium Oxide (HfO2) + Zirconium Carbide (ZrC) logistics modules."""
import os, re
...
def build_records(records):
    lines = []
    for r in records:
        lines.append("  [" + ", ".join(f"'{esc(x)}'" for x in r) + "],")
    return "\n".join(lines)

def gen_file(slug, css_prefix, hex_color, icon, title, subtitle, spec_label, spec_unit, recs):
    sv = slug_var(slug)
    rec_data = build_records(recs)
    ...
    lines = [
        ...
        "];",
        "",
        f"export default function {slug.title().replace('-','_')}LogisticsView() {{",
        "  const [searchTerm, setSearchTerm] = useState('');",
        "  const [statusFilter, setStatusFilter] = useState('all');",
        "  const [activeTab, setActiveTab] = useState('dashboard');",
        "",
        f"  const filtered = useMemo(() => {{",
        f"    return {sv}_RECORDS.filter(r => {{",
        ...
        "    const avgPurity = filtered.reduce((s, r) => s + parseFloat(String(r[6]).replace('%', '')), 0) / (total || 1);",
        ...
        f'        <div className="p-2 rounded-lg" style={{{{ backgroundColor: \'{hex_color}22\' }}}}>',
        f'          <{icon} className="w-5 h-5" style={{{{ color: \'{hex_color}\' }}}} />',
        "        </div>",
        "        <div>",
        f"          <h2 className=\"text-xl font-bold\">{esc(title)}</h2>",
        f"          <p className=\"text-sm text-gray-400\">{esc(subtitle)}</p>",
        "        </div>",
        "      </div>",
        "",
        '      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">',
        "        {[",
        f"          {{ label: 'Total Batches', value: kpis.total, color: '{hex_color}' }},",
        f"          {{ label: 'Investment (Cr)', value: kpis.invest, color: '{hex_color}' }},",
        "          { label: 'Delayed', value: kpis.delayed, color: '#ef4444' },",
        f"          {{ label: 'Avg Purity', value: kpis.avgPurity + '%', color: '{hex_color}' }},",
        "        ].map(k => (",
        ...
        f"                  <div className=\"h-2 rounded-full\" style={{{{ backgroundColor: '{hex_color}', width: `${{(count / kpis.total * 100).toFixed(0)}}%` }}}} />",
        "                </div>",
        ...
        '                <th className="px-4 py-3 text-left text-gray-400 font-medium">Application</th>',
        ...
        "          </div>",
        '          <div className="rounded-xl p-4 border border-gray-700/50 bg-gray-800/30">',
        f'            <h3 className="text-sm font-semibold mb-3 text-gray-300">{esc(spec_label)} Distribution</h3>',
        ...
        "                </div>",
        f'                <span className="text-xs text-gray-400">{{r[7]}} {esc(spec_unit)}</span>',
        "              </div>",
        "            ))}",
        "          </div>",
        "        </div>",
...
# ===== MODULE 1: Hafnium Oxide (HfO2) =====
# High-k gate dielectric, DRAM capacitor, optical coating, nuclear control rod
hfo2_hex = "#0ea5e9"  # sky blue
hfo2_icon = "Hexagon"
hfo2_title = "Hafnium Oxide Logistics"
hfo2_subtitle = "HfO2 high-k dielectric &#8226; DRAM capacitor &#8226; Optical coating &#8226; Nuclear control supply chain"
hfo2_spec = "Dielectric Constant"
hfo2_spec_unit = "k-value"

hfo2_records = [
  ("HFO-A2401","B24-HFO-001","Noida","MIDHANI","HfO2 99.99% Gate Dielectric","FinFET High-k","99.99%","25 k-value","&#8377;940 Cr","in-transit","critical","Noida","Bengaluru","2024-07-15","3","North"),
  ("HFO-A2402","B24-HFO-002","Bengaluru","DRDO DMRL","HfO2 99.9% DRAM Capacitor","CMOS Memory","99.9%","22 k-value","&#8377;900 Cr","delivered","high","Bengaluru","Bengaluru","2024-07-10","1","South"),
  ("HFO-A2403","B24-HFO-003","Mumbai","Tata Electronics","HfO2 99.95% Anti-Reflective","Solar Cell AR","99.95%","24 k-value","&#8377;920 Cr","in-transit","critical","Mumbai","Pune","2024-07-18","2","West"),
  ("HFO-A2404","B24-HFO-004","Chennai","Bharat Forge","HfO2 99.5% Optical Coating","IR Lens Coat","99.5%","20 k-value","&#8377;740 Cr","delivered","medium","Chennai","Chennai","2024-07-08","0","South"),
  ("HFO-A2405","B24-HFO-005","Hyderabad","Shyam Ceramics","HfO2 99.8% Laser Optic","DIRCM Window","99.8%","23 k-value","&#8377;880 Cr","in-transit","critical","Hyderabad","Visakhapatnam","2024-07-20","5","South"),
  ("HFO-A2406","B24-HFO-006","Kolkata","BHEL R&D","HfO2 99.0% Thermal Barrier","GT Blade TBC","99.0%","18 k-value","&#8377;720 Cr","delivered","high","Kolkata","Kolkata","2024-07-12","1","East"),
  ("HFO-A2407","B24-HFO-007","Pune","Godrej Ceramics","HfO2 99.7% Fuel Cell YSZ","SOFC Electrolyte","99.7%","21 k-value","&#8377;860 Cr","in-transit","critical","Pune","Mumbai","2024-07-16","2","West"),
  ("HFO-A2408","B24-HFO-008","Jaipur","Rajasthan Ceramics","HfO2 98.5% Catalyst","Petrochemical","98.5%","17 k-value","&#8377;680 Cr","delivered","medium","Jaipur","Gurugram","2024-07-09","2","North"),
  ("HFO-A2409","B24-HFO-009","Guwahati","Assam Ceramics","HfO2 99.6% Piezoelectric","Sonar Transducer","99.6%","22 k-value","&#8377;840 Cr","in-transit","high","Guwahati","Kolkata","2024-07-22","4","East"),
  ("HFO-A2410","B24-HFO-010","Ahmedabad","Gujarat Ceramics","HfO2 99.98% Space Mirror","ISRO Coating","99.98%","25 k-value","&#8377;960 Cr","pending","critical","Ahmedabad","Sriharikota","2024-07-25","6","West"),
  ("HFO-A2411","B24-HFO-011","Lucknow","UP Ceramics","HfO2 99.3% Nuclear Absorber","Control Rod","99.3%","19 k-value","&#8377;800 Cr","delivered","high","Lucknow","Kalpakkam","2024-07-11","3","North"),
  ("HFO-A2412","B24-HFO-012","Visakhapatnam","Vizag Ceramics","HfO2 99.9% Submarine TPS","Reactor Shield","99.9%","24 k-value","&#8377;940 Cr","delayed","critical","Visakhapatnam","Visakhapatnam","2024-07-06","28","South"),
  ("HFO-A2413","B24-HFO-013","Balasore","DRDO TBRL","HfO2 99.7% Missile Dome","Radome AR Coat","99.7%","22 k-value","&#8377;880 Cr","in-transit","critical","Balasore","Chandipur","2024-07-19","2","East"),
  ("HFO-A2414","B24-HFO-014","Bhilai","SAIL Ceramics","HfO2 97% General Ceramics","Foundry Mold","97.0%","16 k-value","&#8377;640 Cr","delivered","low","Bhilai","Bhilai","2024-07-05","0","East"),
]

print("=== R432a: Hafnium Oxide Logistics ===")
hfo2_path = gen_file("hafnium-oxide", "hfo", hfo2_hex, hfo2_icon, hfo2_title, hfo2_subtitle, hfo2_spec, hfo2_spec_unit, hfo2_records)

# ===== MODULE 2: Zirconium Carbide (ZrC) =====
# Ultra-hard refractory, nuclear fuel coating, hypersonic TPS, cutting tools
zrc_hex = "#a855f7"  # purple
zrc_icon = "Diamond"
zrc_title = "Zirconium Carbide Logistics"
zrc_subtitle = "ZrC ultra-hard refractory &#8226; Nuclear coating &#8226; Hypersonic TPS &#8226; Cutting tool supply chain"
zrc_spec = "Melting Point"
zrc_spec_unit = "degC"
...
print("\n=== R432b: Zirconium Carbide Logistics ===")
zrc_path = gen_file("zirconium-carbide", "zrc", zrc_hex, zrc_icon, zrc_title, zrc_subtitle, zrc_spec, zrc_spec_unit, zrc_records)

print(f"\n=== R432 Generation Complete ===")
print(f"  hafnium-oxide: {hfo2_path}")
print(f"  zirconium-carbide: {zrc_path}")
