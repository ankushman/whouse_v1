#!/usr/bin/env python3
"""R359: Generate Cheriyal Scroll Art Telangana (new) + Textile Apparel Logistics (overwrite) at exactly 253 lines."""

import os

def gen_module(lines_content):
    """Pad or trim content to exactly 253 lines."""
...
# ============================================================
# MODULE 1: Cheriyal Scroll Art Telangana Logistics (NEW)
# ============================================================
cheriyal_content = r"""import React, { useState, useMemo } from 'react'
...
cheriyal_final = gen_module(cheriyal_content)
cheriyal_path = '/home/z/my-project/src/components/modules/cheriyal-scroll-art-telangana-logistics-view.tsx'
with open(cheriyal_path, 'wb') as f:
    f.write(cheriyal_final.encode('utf-8'))
print(f"Cheriyal: {len(cheriyal_final.split(chr(10)))} lines, newlines: {cheriyal_final.count(chr(10))}")


# ============================================================
# MODULE 2: Textile Apparel Logistics (OVERWRITE 234→253)
# ============================================================
textile_content = r"""import React, { useState, useMemo } from 'react'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { PageHeader } from '@/components/shared/page-header'
import { SearchFilterToolbar } from '@/components/shared/search-filter-toolbar'
...
textile_final = gen_module(textile_content)
textile_path = '/home/z/my-project/src/components/modules/textile-apparel-logistics-view.tsx'
with open(textile_path, 'wb') as f:
    f.write(textile_final.encode('utf-8'))
print(f"Textile: {len(textile_final.split(chr(10)))} lines, newlines: {textile_final.count(chr(10))}")

print("\n=== VERIFICATION ===")
for p in [cheriyal_path, textile_path]:
    with open(p, 'rb') as f:
        content = f.read().decode('utf-8')
    lines = content.split('\n')
    newlines = content.count('\n')
    print(f"{os.path.basename(p)}: {len(lines)} lines, {newlines} newlines, assert 253: {newlines == 253}")
