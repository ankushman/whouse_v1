// Generation script for R195 Maritime Cargo Security & Surveillance module
// This script writes the full module file
...
function generateData() {
  ...
  const operators = [
    "Vikram Patel", "Ananya Sharma", "Rajesh Kumar", "Priya Iyer",
    "Suresh Menon", "Deepika Nair", "Arun Singh", "Kavita Desai",
    "Manoj Gupta", "Sneha Reddy",
  ];

  const zones = [
    "Berth Zone A", "Berth Zone B", "Container Yard Alpha", "Container Yard Beta",
    "Customs Inspection Bay", "Gate Complex North", "Gate Complex South",
    "Reefer Station", "Hazmat Storage", "Bonded Warehouse",
    "Port Entry Checkpoint", "Vessel Boarding Area", "CFS Terminal",
    "Cold Storage Zone", "Open Yard Storage",
  ];

  const inspectors = [
    "Inspector Rao", "Officer Mehta", "Commander Joshi",
    "Sub-Inspector Das", "Inspector Verma", "Officer Kulkarni",
  ];
  ...
  const scans: ScanRecord[] = Array.from({ length: 80 }, (_, i) => {
    ...
    return {
      ...
      result: ri() < 0.7 ? "Clear" : ri() < 0.85 ? "Flagged" : ri() < 0.95 ? "Held" : "Rejected",
      ...
      anomalyScore: threatIdx === 0 ? Math.floor(ri() * 15) + 85 : threatIdx === 1 ? Math.floor(ri() * 20) + 60 : threatIdx === 2 ? Math.floor(ri() * 30) + 30 : Math.floor(ri() * 20) + 5,
  ...
  const feeds: SurveillanceFeed[] = Array.from({ length: 50 }, (_, i) => ({
    ...
    cameraName: \`${zones[Math.floor(ri() * zones.length)]} Cam \${Math.floor(ri() * 8) + 1}\`,
    cameraType: CAMERA_TYPES[Math.floor(ri() * CAMERA_TYPES.length)],
    zone: zones[Math.floor(ri() * zones.length)],
  ...
  // ── Security Zones (15) ──
  const securityZones: SecurityZone[] = zones.map((z, i) => ({
  ...
  const incidents: Incident[] = Array.from({ length: 60 }, (_, i) => ({
    ...
    location: zones[Math.floor(ri() * zones.length)],
  ...
  const inspections: Inspection[] = Array.from({ length: 50 }, (_, i) => ({
    ...
    timestamp: \`2024-12-\${String(Math.floor(ri() * 28) + 1).padStart(2, "0")} \${String(Math.floor(ri() * 24)).padStart(2, "0")}:\${String(Math.floor(ri() * 60)).padStart(2, "0")}\`,
    findings: ri() < 0.65 ? "No anomalies detected. Cargo matches declaration." : ri() < 0.8 ? "Minor weight variance. Further review recommended." : "Significant discrepancy found. Cargo flagged for detailed examination.",
    port: INDIAN_PORTS[Math.floor(ri() * INDIAN_PORTS.length)],
    cargoWeight: \`\${(Math.floor(ri() * 28000) + 2000).toLocaleString()} kg\`,
    declaredWeight: \`\${(Math.floor(ri() * 28000) + 2000).toLocaleString()} kg\`,
  ...
  return (
    <div className="mcs-threat-ring relative flex items-center justify-center" style={{ width: size, height: size }}>
      <svg viewBox="0 0 36 36" className="absolute inset-0 w-full h-full -rotate-90">
        ...
        <circle cx="18" cy="18" r="15.9" fill="none" stroke={color} strokeWidth="2.5" strokeDasharray={\`\${level === "Critical" ? 100 : level === "High" ? 75 : level === "Medium" ? 50 : level === "Low" ? 25 : 5}, 100\`} strokeLinecap="round" className="mcs-ring-progress" />
      </svg>
...
function AnomalyScoreBar({ score }: { score: number }) {
  const barColor = score >= 80 ? "from-red-500 to-red-600" : score >= 60 ? "from-orange-400 to-orange-500" : score >= 30 ? "from-amber-400 to-amber-500" : "from-emerald-400 to-emerald-500";
  return (
    <div className="mcs-anomaly-bar flex items-center gap-2">
      <div className="h-2 flex-1 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
        <div className={cn("h-full rounded-full bg-gradient-to-r mcs-anomaly-fill", barColor)} style={{ width: \`\${Math.min(score, 100)}%\` }} />
      </div>
      <span className="text-xs font-mono font-semibold min-w-[2.5rem] text-right" style={{ color: score >= 80 ? "#dc2626" : score >= 60 ? "#ea580c" : score >= 30 ? "#d97706" : "#16a34a" }}>
        {score}%
      </span>
    </div>
  );
...
function CameraStatusIndicator({ status, recording }: { status: string; recording: boolean }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-xs text-slate-500 dark:text-slate-400">{status}{recording ? " ●REC" : ""}</span>
    </div>
  );
}
    ...
    <div className="mcs-compliance-gauge flex flex-col items-center">
      <div className="relative h-12 w-12">
        <svg viewBox="0 0 40 40" className="w-full h-full -rotate-90">
          ...
          <circle cx="20" cy="20" r="18" fill="none" stroke={color} strokeWidth="3" strokeDasharray={\`\${circumference}\`} strokeDashoffset={\`\${offset}\`} strokeLinecap="round" className="mcs-compliance-arc" />
        </svg>
...
function WeightVarianceBadge({ variance }: { variance: number }) {
  const color = variance <= 2 ? "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-950/30" : variance <= 5 ? "text-amber-600 bg-amber-50 dark:text-amber-400 dark:bg-amber-950/30" : variance <= 10 ? "text-orange-600 bg-orange-50 dark:text-orange-400 dark:bg-orange-950/30" : "text-red-600 bg-red-50 dark:text-red-400 dark:bg-red-950/30";
  return (
...
// Drawer Components
// ──────────────────────────────────────────────────────────────
function ScanDrawer({ data, fields }: { data: ScanRecord; fields: { label: string; value: string }[] }) {
  return (
    <>
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-200 dark:border-slate-700">
          <Button size="sm" className="flex-1 bg-cyan-600 hover:bg-cyan-700 text-white" onClick={() => toast.success("Scan report downloaded")}><Download className="h-4 w-4 mr-1" /> Report</Button>
          <Button size="sm" variant="outline" className="flex-1" onClick={() => toast.success("Re-scan initiated")}><ScanLine className="h-4 w-4 mr-1" /> Re-scan</Button>
          <Button size="sm" variant="outline" onClick={() => toast.success("Flag escalated")}><AlertTriangle className="h-4 w-4" /></Button>
        </div>
      </div>
    </>
  );
...
function FeedDrawer({ data, fields }: { data: SurveillanceFeed; fields: { label: string; value: string }[] }) {
  return (
    <>
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-200 dark:border-slate-700">
          <Button size="sm" className="flex-1 bg-slate-700 hover:bg-slate-800 text-white" onClick={() => toast.success("Live feed opened")}><Eye className="h-4 w-4 mr-1" /> Live View</Button>
          <Button size="sm" variant="outline" className="flex-1" onClick={() => toast.success("Recording snapshot saved")}><Download className="h-4 w-4 mr-1" /> Snapshot</Button>
          <Button size="sm" variant="outline" onClick={() => toast.success("PTZ control activated")}><Radio className="h-4 w-4" /></Button>
        </div>
      </div>
    </>
  );
...
function ZoneDrawer({ data, fields }: { data: SecurityZone; fields: { label: string; value: string }[] }) {
  return (
    <>
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-200 dark:border-slate-700">
          <Button size="sm" className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white" onClick={() => toast.success("Patrol dispatched")}><ShieldHalf className="h-4 w-4 mr-1" /> Patrol</Button>
          <Button size="sm" variant="outline" className="flex-1" onClick={() => toast.success("Zone audit started")}><FileSearch className="h-4 w-4 mr-1" /> Audit</Button>
          <Button size="sm" variant="outline" onClick={() => toast.success("Alert issued for zone")}><Siren className="h-4 w-4" /></Button>
        </div>
      </div>
    </>
  );
...
function IncidentDrawer({ data, fields }: { data: Incident; fields: { label: string; value: string }[] }) {
  return (
    <>
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-200 dark:border-slate-700">
          <Button size="sm" className="flex-1 bg-red-600 hover:bg-red-700 text-white" onClick={() => toast.success("Incident escalated")}><Siren className="h-4 w-4 mr-1" /> Escalate</Button>
          ...
          <Button size="sm" variant="outline" onClick={() => toast.success("Incident resolved")}><CheckCircle2 className="h-4 w-4" /></Button>
        </div>
      </div>
    </>
  );
...
function InspectionDrawer({ data, fields }: { data: Inspection; fields: { label: string; value: string }[] }) {
  return (
    <>
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-200 dark:border-slate-700">
          <Button size="sm" className="flex-1 bg-teal-600 hover:bg-teal-700 text-white" onClick={() => toast.success("Inspection report generated")}><Download className="h-4 w-4 mr-1" /> Report</Button>
          <Button size="sm" variant="outline" className="flex-1" onClick={() => toast.success("Re-inspection scheduled")}><ScanLine className="h-4 w-4 mr-1" /> Re-inspect</Button>
          <Button size="sm" variant="outline" onClick={() => toast.success("Quarantine hold placed")}><ShieldHalf className="h-4 w-4" /></Button>
        </div>
      </div>
    </>
  );
...
export default function MaritimeSecurityView() {
  ...
  const openIncidents = data.incidents.filter(i => i.status === "Open" || i.status === "Investigating" || i.status === "Escalated").length;
  const avgCompliance = Math.round(data.securityZones.reduce((a, z) => a + z.compliance, 0) / data.securityZones.length);
  ...
  // ── Filter & Sort logic per tab ──
  const filteredScans = useMemo(() => {
    let filtered = data.scans;
    if (searchTerm) filtered = filtered.filter(s => s.id.toLowerCase().includes(searchTerm.toLowerCase()) || s.containerId.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const filteredFeeds = useMemo(() => {
    ...
    if (searchTerm) filtered = filtered.filter(f => f.cameraName.toLowerCase().includes(searchTerm.toLowerCase()) || f.zone.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const filteredIncidents = useMemo(() => {
    ...
    if (searchTerm) filtered = filtered.filter(i => i.id.toLowerCase().includes(searchTerm.toLowerCase()) || i.type.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const filteredInspections = useMemo(() => {
    ...
    if (searchTerm) filtered = filtered.filter(ins => ins.id.toLowerCase().includes(searchTerm.toLowerCase()) || ins.containerId.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="mcs-root flex flex-col gap-4 p-4 md:p-6">
      <PageHeader title="Maritime Cargo Security & Surveillance" description="Port security operations, cargo scanning, surveillance monitoring & incident management across Indian ports" />
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="mcs-tabs flex flex-wrap gap-1 bg-slate-100 dark:bg-slate-800 rounded-lg p-1 h-auto">
          {["Security Dashboard", "Cargo Scanning", "Surveillance Network", "Security Zones", "Incident Tracker", "Inspection Audit"].map((tab, idx) => (
            <TabsTrigger key={idx} value={String(idx)} className="mcs-tab-trigger">{tab}</TabsTrigger>
          ))}
        </TabsList>
        ...
        <TabsContent value="0" className="space-y-4 mt-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="mcs-chart-card border-0 shadow-sm">
              <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Threat Level Trend</CardTitle></CardHeader>
              <CardContent>
            ...
            <Card className="mcs-chart-card border-0 shadow-sm">
              <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Monthly Scan Volume</CardTitle></CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={260}>
            ...
            <Card className="mcs-chart-card border-0 shadow-sm">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={260}>
                  <PieChart>
                    <Pie data={data.incidentByType} dataKey="count" nameKey="type" cx="50%" cy="50%" outerRadius={90} innerRadius={50} label={({ type, count }: any) => \`\${count}\`} labelLine={false}>
                      {data.incidentByType.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                    </Pie>
                    <Tooltip contentStyle={{ borderRadius: 8, fontSize: 12 }} />
                  </PieChart>
                </ResponsiveContainer>
            ...
            <Card className="mcs-chart-card border-0 shadow-sm">
              <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Zone Compliance Overview</CardTitle></CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={data.zoneCompliance} layout="vertical">
                    ...
                    <Bar dataKey="compliance" radius={[0, 4, 4, 0]}>
                      {data.zoneCompliance.map((entry, i) => (
                        <Cell key={i} fill={entry.compliance >= 95 ? "#10b981" : entry.compliance >= 85 ? "#06b6d4" : entry.compliance >= 75 ? "#d97706" : "#ef4444"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
        ...
        <TabsContent value="1" className="space-y-4 mt-4">
          <div className="flex flex-wrap gap-2 items-center">
            <Input placeholder="Search scan ID or container..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-64 h-8 text-sm" />
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="h-8 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-2 text-sm">
              ...
              {data.INSPECTION_RESULTS.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800">
                  <th className="mcs-sort-header px-3 py-2 text-left cursor-pointer hover:text-cyan-600" onClick={() => handleSort("id")}>Scan ID {sortBy === "id" && (sortDir === "asc" ? "↑" : "↓")}</th>
              ...
              <tbody>
                {filteredScans.slice(0, 20).map((scan) => (
                  <tr key={scan.id} className="mcs-table-row border-t border-slate-100 dark:border-slate-800 hover:bg-cyan-50/50 dark:hover:bg-cyan-950/20">
                    ...
                    <td className="px-3 py-2"><span className={cn("inline-block rounded-full px-2 py-0.5 text-xs font-semibold", inspectionColorMap[scan.result])}>{scan.result}</span></td>
                    ...
                    <td className="px-3 py-2 text-center">
                      <Button size="sm" variant="ghost" className="mcs-action-btn h-7 px-2 text-xs" onClick={() => openDrawer("scan", scan)}>View</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <TabsContent value="2" className="space-y-4 mt-4">
          <div className="flex flex-wrap gap-2 items-center">
            <Input placeholder="Search camera or zone..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-64 h-8 text-sm" />
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="h-8 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-2 text-sm">
              <option value="All">All Status</option>
              {["Online", "Alert", "Offline"].map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredFeeds.slice(0, 18).map(feed => (
              <Card key={feed.id} className="mcs-feed-card border-0 shadow-sm hover:shadow-md transition-all cursor-pointer" onClick={() => openDrawer("feed", feed)}>
        ...
        <TabsContent value="3" className="space-y-4 mt-4">
          <div className="flex flex-wrap gap-2 items-center">
            <Input placeholder="Search zone..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-64 h-8 text-sm" />
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="h-8 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-2 text-sm">
              <option value="All">All Status</option>
              {data.ZONE_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredZones.map(zone => (
              <Card key={zone.id} className="mcs-zone-card border-0 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer" onClick={() => openDrawer("zone", zone)}>
        ...
        <TabsContent value="4" className="space-y-4 mt-4">
          <div className="flex flex-wrap gap-2 items-center">
            <Input placeholder="Search incident ID or type..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-64 h-8 text-sm" />
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="h-8 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-2 text-sm">
              <option value="All">All Severity</option>
              {data.THREAT_LEVELS.slice(0, 4).map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div className="rounded-lg border overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800">
                  <th className="mcs-sort-header px-3 py-2 text-left cursor-pointer hover:text-red-600" onClick={() => handleSort("id")}>ID {sortBy === "id" && (sortDir === "asc" ? "↑" : "↓")}</th>
              ...
              <tbody>
                {filteredIncidents.slice(0, 20).map(inc => (
                  <tr key={inc.id} className="mcs-table-row border-t border-slate-100 dark:border-slate-800 hover:bg-red-50/50 dark:hover:bg-red-950/20">
                    ...
                    <td className="px-3 py-2"><span className={cn("inline-block rounded-full px-2 py-0.5 text-xs font-bold", threatColorMap[inc.severity])}>{inc.severity}</span></td>
                    <td className="px-3 py-2">
                      <span className={cn("text-xs font-semibold", inc.status === "Open" ? "text-red-600" : inc.status === "Resolved" || inc.status === "Closed" ? "text-emerald-600" : "text-amber-600")}>{inc.status}</span>
                    </td>
                    ...
                    <td className="px-3 py-2 text-center">
                      <Button size="sm" variant="ghost" className="mcs-action-btn h-7 px-2 text-xs" onClick={() => openDrawer("incident", inc)}>View</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <TabsContent value="5" className="space-y-4 mt-4">
          <div className="flex flex-wrap gap-2 items-center">
            <Input placeholder="Search inspection ID or container..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-64 h-8 text-sm" />
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="h-8 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-2 text-sm">
              <option value="All">All Results</option>
              {data.INSPECTION_RESULTS.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
          ...
          <div className="rounded-lg border overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800">
                  <th className="mcs-sort-header px-3 py-2 text-left cursor-pointer hover:text-teal-600" onClick={() => handleSort("id")}>ID {sortBy === "id" && (sortDir === "asc" ? "↑" : "↓")}</th>
                  <th className="px-3 py-2 text-left">Container</th>
              ...
              <tbody>
                {filteredInspections.slice(0, 20).map(ins => (
                  <tr key={ins.id} className="mcs-table-row border-t border-slate-100 dark:border-slate-800 hover:bg-teal-50/50 dark:hover:bg-teal-950/20">
                    ...
                    <td className="px-3 py-2"><span className={cn("inline-block rounded-full px-2 py-0.5 text-xs font-semibold", inspectionColorMap[ins.result])}>{ins.result}</span></td>
                    ...
                    <td className="px-3 py-2 text-center">
                      <Button size="sm" variant="ghost" className="mcs-action-btn h-7 px-2 text-xs" onClick={() => openDrawer("inspection", ins)}>View</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <SheetContent className="w-[420px] overflow-y-auto p-4 sm:p-6">
          ...
          <div className="mt-4">
            {drawerType === "scan" && drawerData && (
              <ScanDrawer data={drawerData} fields={[
            ...
            {drawerType === "feed" && drawerData && (
              <FeedDrawer data={drawerData} fields={[
            ...
            {drawerType === "zone" && drawerData && (
              <ZoneDrawer data={drawerData} fields={[
            ...
            {drawerType === "incident" && drawerData && (
              <IncidentDrawer data={drawerData} fields={[
            ...
            {drawerType === "inspection" && drawerData && (
              <InspectionDrawer data={drawerData} fields={[
...
fs.writeFileSync('/home/z/my-project/src/components/modules/maritime-cargo-security-view.tsx', content);
console.log('✅ Module file created: maritime-cargo-security-view.tsx');
